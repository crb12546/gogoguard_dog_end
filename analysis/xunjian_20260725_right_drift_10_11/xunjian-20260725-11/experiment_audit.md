...
- 生成时间：2026-07-25T16:55:59.648+08:00
- 运行目录：`/home/unitree/go2_fastlio_ws/patrol_logs/runs/20260725/xunjian-20260725-11`
- 证据流是否完整：是
- 完整体检材料是否齐全：否
...
- [warning] odometry_continuity：the canonical bag contains an elevated odometry gap
...
- [ok] route_tracking：FAST-LIO trajectory stayed close to the runtime CSV
...
- 坐标转换可精确复算：是
- 起步位置误差：0.015 m；起步朝向误差：0.008°
- 全程横向误差：中位 0.105 m，P95 0.173 m，最大 0.225 m
- 前 5m 横向误差：P95 0.098 m，最大 0.105 m
- 跟线器逐周期证据：7078 条控制决策、使用 3394 个里程计回调；同一里程计连续复用最多 9 个控制周期；里程计时间戳年龄 P95 310.3 ms
- 全速 rosbag 关键话题：齐全；各话题消息数={"/Odometry": 3702, "/api/sport/request": 7690, "/cmd_vel": 7687, "/lf/sportmodestate": 7785, "/livox/imu": 77729, "/patrol_cmd": 7603, "/tf": 3702}
- rosbag 里程计最大记录间隔：0.429 s
- FAST-LIO 活性事件：自动恢复 0 次，健康失败 0 次，永久锁死 0 次
- 安全层把非零命令改成零：41 次（配对命令 4664 条）
- 性能采样：398 条；最高温度 66.4°C；监控唤醒延迟 P95 6.4 ms
...
- FAST-LIO 多核证据：进程 CPU 峰值 99.6%，线程数最多 11，线程实际落到的 CPU=[0, 1, 2, 3, 4, 5, 6, 7]，允许 CPU=["0-7"]
...
- FAST-LIO 与狗身 IMU 相对姿态：可比较；相对 R/P/Y 中位数=11.67°/11.87°/-16.12°；相对姿态变化 P95=33.8°/18.8°/22.25°
- 狗底层：电机最高温度 53°C；电池功率 P95 182.5W；Sport 非零错误 5002 条；motor lost 0 条
