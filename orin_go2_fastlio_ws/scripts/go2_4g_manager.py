...
MANAGER_VERSION = "2026-07-23-patrol-safe-recovery-v6"
XHCI_FATAL_PATTERNS = (
...
PROTECTED_WORKLOADS = {
    ...
    "patrol_relocalizing": "route_relocalizer",
    "patrol_alignment": "check_route_start_alignment.py",
...
class Config:
    ...
    usb_unstable_settle_seconds: int
    usb_absent_rescan_after_seconds: int
    usb_absent_rescan_interval_seconds: int
    radio_off_seconds: int
    state_file: Path
    auto_reboot_on_fatal: bool
    fatal_reboot_delay_seconds: int
    fatal_reboot_max_burst: int
    fatal_reboot_backoff_seconds: int
    fatal_reboot_stable_online_seconds: int
    recovery_job_end_grace_seconds: int
    reboot_marker: Path
    fault_log: Path
    ...
    def from_env(cls) -> "Config":
        return cls(
            ...
            usb_unstable_settle_seconds=env_int(
                "GO2_4G_USB_UNSTABLE_SETTLE_SECONDS", 30),
            usb_absent_rescan_after_seconds=env_int(
                "GO2_4G_USB_ABSENT_RESCAN_AFTER_SECONDS", 120),
            usb_absent_rescan_interval_seconds=env_int(
                "GO2_4G_USB_ABSENT_RESCAN_INTERVAL_SECONDS", 300),
            ...
            fatal_reboot_delay_seconds=env_int(
                "GO2_4G_FATAL_REBOOT_DELAY_SECONDS", 30),
            fatal_reboot_max_burst=env_int(
                "GO2_4G_FATAL_REBOOT_MAX_BURST", 3),
            fatal_reboot_backoff_seconds=env_int(
                "GO2_4G_FATAL_REBOOT_BACKOFF_SECONDS", 900),
            fatal_reboot_stable_online_seconds=env_int(
                "GO2_4G_FATAL_REBOOT_STABLE_ONLINE_SECONDS", 600),
            recovery_job_end_grace_seconds=env_int(
                "GO2_4G_RECOVERY_JOB_END_GRACE_SECONDS", 120),
    ...
    def validate(self) -> None:
        ...
        if min(self.loop_seconds, self.dhcp_retry_seconds, self.health_failures,
               ...
               self.rapid_usb_window, self.usb_unstable_settle_seconds,
               self.usb_absent_rescan_after_seconds,
               self.usb_absent_rescan_interval_seconds,
               self.radio_off_seconds,
               self.fatal_reboot_delay_seconds,
               self.fatal_reboot_max_burst,
               self.fatal_reboot_backoff_seconds,
               self.fatal_reboot_stable_online_seconds,
               self.recovery_job_end_grace_seconds) <= 0:
...
class Manager:
    def __init__(self, config: Config) -> None:
        ...
        self.usb_link: Dict[str, object] = {}
        self.usb_degraded_event_logged = False
        self.usb_protocol_warning_count = 0
        self.usb_protocol_evidence_cache: List[str] = []
        self.usb_instability_event_logged = False
        self.last_usb_root_rescan = -float("inf")
        self.usb_root_rescan_count = 0
        self.last_usb_root_rescan_result = ""
        self.online_since: Optional[float] = None
        self.reboot_budget_reset_logged = False
        self.last_protected_workload_seen: Optional[float] = None
    ...
    def ensure_usb_host_awake(self) -> str:
        ...
        root_hub_interface = (
            self.usb_root / self.cfg.usb_bus_device
            / f"{self.cfg.usb_bus_device.replace('usb', '')}-0:1.0")
        for port_power in root_hub_interface.glob(
                f"{self.cfg.usb_bus_device}-port*/power/control"):
            self.force_power_control_on(port_power, port_power.parent.parent.name)
        return read_text(self.xhci_root / "power/runtime_status")

    def usb_root_rescan_is_safe(self) -> Tuple[bool, List[str]]:
        """Do not disrupt unrelated devices while rescanning the modem bus."""
        bus_number = self.cfg.usb_bus_device.replace("usb", "", 1)
        other_devices: List[str] = []
        try:
            entries = list(self.usb_root.iterdir())
        except OSError:
            return False, ["USB sysfs is unavailable"]
        for entry in entries:
            if ":" in entry.name or not entry.name.startswith(bus_number + "-"):
                continue
            vid = read_text(entry / "idVendor").lower()
            pid = read_text(entry / "idProduct").lower()
            if not vid or not pid:
                continue
            if ((vid, pid) in {
                    (self.cfg.usb_vid, self.cfg.usb_pid),
                    (self.cfg.switch_vid, self.cfg.switch_pid)}):
                continue
            other_devices.append(f"{entry.name}:{vid}:{pid}")
        return not other_devices, other_devices

    def rescan_usb_root_hub(self, now: float) -> bool:
        """Re-probe an electrically present modem without removing XHCI.

        This unbinds only the generic USB root-hub driver.  It does not remove
        the Tegra platform controller or reload Falcon firmware, both of which
        are unsafe on the installed R35.3.1 kernel.
        """
        safe, other_devices = self.usb_root_rescan_is_safe()
        workloads, job_grace_remaining = self.recovery_workload_guard(now)
        self.last_usb_root_rescan = now
        if workloads or job_grace_remaining > 0 or not safe:
            self.last_usb_root_rescan_result = (
                "deferred for active robot work" if workloads
                else "deferred during the post-job safety grace period"
                if job_grace_remaining > 0
                else "deferred because other USB devices share the bus")
            self.set_state(
                "USB_ABSENT",
                "A7600C is absent; root-hub rescan is deferred",
                protected_workloads=workloads,
                post_job_grace_remaining_seconds=round(
                    job_grace_remaining, 1),
                other_usb_devices=other_devices,
                rescan_result=self.last_usb_root_rescan_result,
            )
            return False

        driver_root = self.cfg.sysfs_root / "bus/usb/drivers/usb"
        unbind = driver_root / "unbind"
        bind = driver_root / "bind"
        bus = self.cfg.usb_bus_device
        unbound = False
        self.usb_root_rescan_count += 1
        self.append_fault_event(
            "usb_root_rescan_started",
            usb_bus=bus,
            attempt=self.usb_root_rescan_count,
        )
        self.set_state(
            "USB_ROOT_RESCAN",
            "A7600C remains absent; safely rescanning its empty USB root bus",
            usb_bus=bus,
            attempt=self.usb_root_rescan_count,
        )
        try:
            unbind.write_text(f"{bus}\n", encoding="ascii")
            unbound = True
            time.sleep(3)
            bind.write_text(f"{bus}\n", encoding="ascii")
            unbound = False
        except OSError as exc:
            if unbound:
                try:
                    ...
                    logging.exception("failed to restore USB root hub %s", bus)
            self.last_usb_root_rescan_result = f"failed: {exc}"
            self.append_fault_event(
                "usb_root_rescan_failed",
                usb_bus=bus,
                attempt=self.usb_root_rescan_count,
                error=str(exc),
            )
            logging.error("USB root-hub rescan failed: %s", exc)
            return False
        self.last_usb_root_rescan_result = "completed; waiting for enumeration"
        self.append_fault_event(
            "usb_root_rescan_completed",
            usb_bus=bus,
            attempt=self.usb_root_rescan_count,
        )
        logging.warning(
            "rescanned empty USB root bus %s for the absent A7600C (attempt %s)",
            bus,
            self.usb_root_rescan_count,
        )
        return True
    ...
    def recovery_workload_guard(self, now: float) -> Tuple[List[str], float]:
        """Block disruptive recovery during and immediately after robot jobs."""
        workloads = self.active_workloads()
        if workloads:
            self.last_protected_workload_seen = now
            return workloads, float(self.cfg.recovery_job_end_grace_seconds)
        if self.last_protected_workload_seen is None:
            return [], 0.0
        remaining = max(
            0.0,
            self.cfg.recovery_job_end_grace_seconds
            - (now - self.last_protected_workload_seen),
        )
        return [], remaining
    ...
    def usb_protocol_evidence(self, device_name: str) -> List[str]:
        pattern = re.compile(
            (r"\busb\s+%s:.*(?:error\s+-71|Device not responding|"
             r"not accepting address|device descriptor read|not running at top speed)")
            % re.escape(device_name),
            re.IGNORECASE,
        )
        return [line.strip() for line in self.kernel_log_text().splitlines()
                if pattern.search(line)][-30:]

    def update_usb_link_diagnostics(self, device: Path) -> None:
        speed_text = read_text(device / "speed")
        diagnostics: Dict[str, object] = {
            "device": device.name,
            "speed_mbps": None,
            "expected_high_speed_mbps": 480,
            "degraded": False,
            "max_power": read_text(device / "bMaxPower") or "unknown",
        }
        try:
            ...
            speed = 0.0
        if speed > 0:
            diagnostics["speed_mbps"] = int(speed) if speed.is_integer() else speed
            diagnostics["degraded"] = speed < 480.0
        if diagnostics["degraded"]:
            if not self.usb_degraded_event_logged:
                self.usb_protocol_evidence_cache = self.usb_protocol_evidence(
                    device.name)
                self.usb_protocol_warning_count = len(
                    self.usb_protocol_evidence_cache)
            diagnostics["protocol_warning_count"] = self.usb_protocol_warning_count
            diagnostics["kernel_evidence"] = self.usb_protocol_evidence_cache[-8:]
        if diagnostics["degraded"] and not self.usb_degraded_event_logged:
            self.append_fault_event(
                "usb_link_degraded",
                reason="A7600C negotiated below USB 2.0 high speed",
                usb_link=diagnostics,
                kernel_evidence=self.usb_protocol_evidence_cache,
                recent_resources=list(self.resource_samples),
            )
            logging.warning(
                "A7600C USB link is degraded: speed=%s Mbps expected=480 Mbps; "
                "protocol_warnings=%s",
                diagnostics["speed_mbps"],
                self.usb_protocol_warning_count,
            )
            self.usb_degraded_event_logged = True
        self.usb_link = diagnostics
    ...
    def read_reboot_marker(self) -> Dict[str, object]:
        try:
    ...
    def fatal_reboot_policy(self, fatal_age: float) -> Dict[str, object]:
        """Rate-limit recovery without trusting the Jetson's stale RTC.

        The previous implementation compared wall-clock timestamps.  On a
        disconnected boot this Jetson can start with a 2022 clock, so a marker
        written after network time sync looked as if it came from the future
        and blocked recovery forever.  Count completed boot attempts instead.
        After a short burst, use the current boot's monotonic fatal age as the
        backoff clock.
        """
        marker = self.read_reboot_marker()
        try:
            attempts = max(
                0, int(marker.get("consecutive_requests", 1 if marker else 0)))
        except (TypeError, ValueError):
            attempts = 1 if marker else 0
        marker_boot_id = str(marker.get("boot_id", ""))
        current_boot_id = read_text(Path("/proc/sys/kernel/random/boot_id"))
        same_boot = bool(marker_boot_id and marker_boot_id == current_boot_id)
        backoff_remaining = 0.0
        if attempts >= self.cfg.fatal_reboot_max_burst:
            backoff_remaining = max(
                0.0, self.cfg.fatal_reboot_backoff_seconds - fatal_age)
        return {
            "attempts": attempts,
            "same_boot": same_boot,
            "backoff_remaining": backoff_remaining,
            "allowed": not same_boot and backoff_remaining <= 0,
        }

    def write_reboot_marker(self, reason: str, previous_attempts: int) -> int:
        attempt = min(
            self.cfg.fatal_reboot_max_burst,
            max(0, previous_attempts) + 1,
        )
        payload = {
            ...
            "boot_id": read_text(Path("/proc/sys/kernel/random/boot_id")),
            "reason": reason,
            "consecutive_requests": attempt,
            "max_burst": self.cfg.fatal_reboot_max_burst,
        }
    ...
    def note_internet_healthy(self, now: float) -> None:
        if self.online_since is None:
            self.online_since = now
            self.reboot_budget_reset_logged = False
            return
        online_age = now - self.online_since
        if (online_age < self.cfg.fatal_reboot_stable_online_seconds or
                self.reboot_budget_reset_logged or
                not self.cfg.reboot_marker.exists()):
            return
        try:
            self.cfg.reboot_marker.unlink()
        except FileNotFoundError:
            pass
        except OSError as exc:
            logging.warning("cannot clear fatal reboot history: %s", exc)
            return
        self.reboot_budget_reset_logged = True
        self.append_fault_event(
            "fatal_reboot_budget_reset",
            stable_online_seconds=round(online_age, 1),
        )
        logging.info(
            "cleared fatal reboot history after %.1fs of stable internet",
            online_age,
        )
    ...
    def handle_fatal_controller(self, now: float, controller_status: str) -> None:
        ...
        workloads, job_grace_remaining = self.recovery_workload_guard(now)
        reboot_policy = self.fatal_reboot_policy(fatal_age)
        attempts = int(reboot_policy["attempts"])
        backoff_remaining = float(reboot_policy["backoff_remaining"])
        ...
        elif job_grace_remaining > 0:
            recovery = "automatic reboot deferred during post-job safety grace"
        ...
        elif bool(reboot_policy["same_boot"]):
            recovery = "controlled reboot was already requested in this boot"
        elif backoff_remaining > 0:
            recovery = "bounded reboot backoff active after repeated USB failures"
        else:
            reason = "fatal Tegra XUSB controller failure removed the 4G modem"
            try:
                attempt = self.write_reboot_marker(reason, attempts)
            except OSError as exc:
                logging.error("cannot persist automatic reboot marker: %s", exc)
                recovery = "automatic reboot blocked because history could not be saved"
            else:
                self.reboot_requested = True
                self.append_fault_event(
                    "controlled_reboot_requested",
                    reason=reason,
                    reboot_attempt=attempt,
                    reboot_max_burst=self.cfg.fatal_reboot_max_burst,
                ...
                logging.critical(
                    "requesting controlled reboot %s/%s after fatal Tegra XUSB failure",
                    attempt,
                    self.cfg.fatal_reboot_max_burst,
                )
        ...
        self.set_state(
            ...
            fatal_age_seconds=round(fatal_age, 1),
            reboot_attempts_in_burst=attempts,
            reboot_max_burst=self.cfg.fatal_reboot_max_burst,
            reboot_backoff_seconds=self.cfg.fatal_reboot_backoff_seconds,
            reboot_backoff_remaining_seconds=round(backoff_remaining, 1),
            recovery=recovery,
            protected_workloads=workloads,
            post_job_grace_seconds=self.cfg.recovery_job_end_grace_seconds,
            post_job_grace_remaining_seconds=round(job_grace_remaining, 1),
    ...
    def wait_for_usb_instability_to_settle(self, now: float) -> bool:
        while (self.usb_generation_times and
               now - self.usb_generation_times[0] > self.cfg.rapid_usb_window):
            self.usb_generation_times.popleft()
        generations = len(self.usb_generation_times)
        if generations < self.cfg.rapid_usb_limit:
            return False
        stable_age = max(0.0, now - self.usb_first_seen)
        if not self.usb_instability_event_logged:
            self.append_fault_event(
                "usb_instability_quarantined",
                reason="rapid USB re-enumeration; suppressing AT/radio recovery",
                generations=generations,
                window_seconds=self.cfg.rapid_usb_window,
                settle_seconds=self.cfg.usb_unstable_settle_seconds,
                usb_link=self.usb_link,
            )
            self.usb_instability_event_logged = True
        if stable_age < self.cfg.usb_unstable_settle_seconds:
            self.set_state(
                "USB_UNSTABLE_SETTLING",
                "rapid USB re-enumeration detected; waiting without touching the radio",
                generations=generations,
                window_seconds=self.cfg.rapid_usb_window,
                stable_seconds=round(stable_age, 1),
                settle_seconds=self.cfg.usb_unstable_settle_seconds,
            )
            return True
        logging.warning(
            "A7600C USB generation remained stable for %.1fs; resuming normal connection",
            stable_age,
        )
        self.append_fault_event(
            "usb_instability_settled",
            generations=generations,
            stable_seconds=round(stable_age, 1),
            usb_link=self.usb_link,
        )
    ...
    def tick(self) -> None:
        ...
        if device is None:
            ...
            if controller_status in {"error", "unsupported"} or self.xhci_fatal_evidence:
                ...
                absent_age = max(0.0, now - self.usb_absent_since)
                since_rescan = now - self.last_usb_root_rescan
                if (absent_age >= self.cfg.usb_absent_rescan_after_seconds and
                        since_rescan >=
                        self.cfg.usb_absent_rescan_interval_seconds):
                    self.rescan_usb_root_hub(now)
                else:
                    initial_remaining = max(
                        0.0,
                        self.cfg.usb_absent_rescan_after_seconds - absent_age,
                    )
                    interval_remaining = max(
                        0.0,
                        self.cfg.usb_absent_rescan_interval_seconds - since_rescan,
                    )
                    self.set_state(
                        "USB_ABSENT",
                        "A7600C USB device is absent; the awake host is waiting for re-enumeration",
                        controller_runtime_status=controller_status or "unknown",
                        usb_absent_seconds=round(absent_age, 1),
                        next_root_rescan_seconds=round(
                            max(initial_remaining, interval_remaining), 1),
                        root_rescan_count=self.usb_root_rescan_count,
                        last_root_rescan_result=self.last_usb_root_rescan_result,
                    )
            return
        ...
        self.update_usb_link_diagnostics(device)
        ...
        if self.wait_for_usb_instability_to_settle(now):
            return
        ...
        if self.tcp_check(self.cfg.cloud_host, self.cfg.cloud_port):
            ...
            self.dhcp_failures = 0
            self.offline_since = None
            self.note_internet_healthy(now)
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="go2-4g-sysfs-") as temporary:
        ...
        degraded_device = root / "bus/usb/devices/1-2"
        degraded_device.mkdir()
        (degraded_device / "speed").write_text("12\n", encoding="ascii")
        (degraded_device / "bMaxPower").write_text("500mA\n", encoding="ascii")
        ...
        test_manager.update_usb_link_diagnostics(degraded_device)
        assert test_manager.usb_link["speed_mbps"] == 12
        assert test_manager.usb_link["degraded"] is True
        assert test_manager.usb_link["protocol_warning_count"] == 2
        test_manager.update_usb_link_diagnostics(degraded_device)
        assert test_manager.usb_link["protocol_warning_count"] == 2
        test_manager.kernel_log_text = lambda: (  # type: ignore[method-assign]
        ...
        assert [event["event"] for event in events] == [
            "usb_link_degraded", "xhci_fatal_detected"]
        assert test_manager.fatal_reboot_policy(0)["attempts"] == 0
        assert test_manager.write_reboot_marker("self-test", 0) == 1
        policy = test_manager.fatal_reboot_policy(0)
        assert policy["attempts"] == 1
        if read_text(Path("/proc/sys/kernel/random/boot_id")):
            assert policy["same_boot"] is True
        marker = test_manager.read_reboot_marker()
        marker["boot_id"] = "previous-test-boot"
        marker["consecutive_requests"] = test_cfg.fatal_reboot_max_burst
        test_cfg.reboot_marker.write_text(
            json.dumps(marker) + "\n", encoding="utf-8")
        policy = test_manager.fatal_reboot_policy(100)
        assert policy["allowed"] is False
        assert policy["backoff_remaining"] == (
            test_cfg.fatal_reboot_backoff_seconds - 100)
        policy = test_manager.fatal_reboot_policy(
            test_cfg.fatal_reboot_backoff_seconds + 1)
        assert policy["allowed"] is True
        test_manager.online_since = (
            time.monotonic() - test_cfg.fatal_reboot_stable_online_seconds - 1)
        test_manager.note_internet_healthy(time.monotonic())
        assert not test_cfg.reboot_marker.exists()
        now = time.monotonic()
        test_manager.usb_generation_times = deque(
            [now - 2, now - 1, now])
        test_manager.usb_first_seen = now
        assert test_manager.wait_for_usb_instability_to_settle(now) is True
        assert test_manager.state == "USB_UNSTABLE_SETTLING"
        test_manager.usb_first_seen = (
            now - test_cfg.usb_unstable_settle_seconds - 1)
        assert test_manager.wait_for_usb_instability_to_settle(now) is False
        assert not test_manager.usb_generation_times
        test_manager.active_workloads = (  # type: ignore[method-assign]
            lambda: ["patrol"])
        workloads, grace = test_manager.recovery_workload_guard(now)
        assert workloads == ["patrol"]
        assert grace == test_cfg.recovery_job_end_grace_seconds
        test_manager.active_workloads = (  # type: ignore[method-assign]
            lambda: [])
        workloads, grace = test_manager.recovery_workload_guard(now + 1)
        assert not workloads
        assert grace == test_cfg.recovery_job_end_grace_seconds - 1
        workloads, grace = test_manager.recovery_workload_guard(
            now + test_cfg.recovery_job_end_grace_seconds + 1)
        assert not workloads
        assert grace == 0
