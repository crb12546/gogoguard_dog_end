...
def resolve_route_map(route_path, params):
    ...
    route_key = normalized_asset_key(Path(route_path).stem)
    matches = []
    if route_key and PCD_DIR.is_dir():
        for path in PCD_DIR.glob("*.pcd"):
            key = normalized_asset_key(path.stem)
            if not key:
                continue
            if key == route_key:
                score = 0
            elif key.startswith(route_key):
                score = 1
            elif route_key.startswith(key):
                score = 2
            else:
                continue
            try:
                ...
                mtime = 0
            matches.append((score, abs(len(key) - len(route_key)), -mtime, path))
    if matches:
        matches.sort()
        return matches[0][3], "fuzzy:%s" % matches[0][3].stem
    return None, ""
...
def route_relocalization_plan(route_path, params):
    enabled = bool_param(
        params,
        ["relocalize", "useRelocalization", "use_relocalization", "mapRelocalize", "map_relocalize"],
        env_bool("GO2_PATROL_RELOCALIZE", True),
    )
    required = bool_param(
        params,
        ["requireRelocalize", "require_relocalize", "requireRelocalization", "require_relocalization"],
        env_bool("GO2_PATROL_REQUIRE_RELOCALIZE", False),
    )
    plan = {
    ...
    if map_path is None:
        plan["enabled"] = False
        plan["reason"] = "no matching pcd map"
        if required:
            raise RuntimeError("route relocalization required but no matching pcd map for: %s" % Path(route_path).name)
        return plan
...
def start_patrol_command(route_path, params, route_info=None):
    speed = bounded_float(params.get("speed", params.get("v_base", 0.25)), 0.25, 0.05, 0.3)
    start_max_distance = bounded_float(
        params.get("startMaxDistance", params.get("start_max_distance", 0.5)),
        0.5,
        0.2,
        0.8,
    )
    start_max_yaw_error = bounded_float(
        params.get("startMaxYawError", params.get("start_max_yaw_error", 0.9)),
        0.9,
        0.2,
        3.14,
    )
    ...
    recovery_vx = bounded_float(
        params.get("recoveryVx", params.get("recovery_vx", 0.20)),
        0.20,
        0.03,
        0.3,
    )
    max_yaw_rate = bounded_float(
        params.get("maxYawRate", params.get("max_yaw_rate", 0.45)),
        0.45,
        0.2,
        0.5,
    )
    k_yaw = bounded_float(
        params.get("kYaw", params.get("k_yaw", 0.75)),
        0.75,
        0.10,
        1.20,
    )
    lookahead_distance = bounded_float(
        params.get("lookaheadDistance", params.get("lookahead_distance", 0.60)),
        0.60,
        0.25,
        1.2,
    )
    corner_turn_distance = bounded_float(
        params.get("cornerTurnDistance", params.get("corner_turn_distance", 0.35)),
        0.35,
        0.20,
        1.50,
    )
    turn_in_place_angle = bounded_float(
        params.get("turnInPlaceAngle", params.get("turn_in_place_angle", 1.35)),
        1.35,
        0.6,
        2.4,
    )
    slow_down_angle = bounded_float(
        params.get("slowDownAngle", params.get("slow_down_angle", 0.75)),
        0.75,
        0.2,
        1.5,
    )
    lateral_yaw_gain = bounded_float(
        params.get("lateralYawGain", params.get("lateral_yaw_gain", params.get("lateralCorrectionGain", params.get("lateral_correction_gain", 0.18)))),
        0.18,
        0.0,
        1.0,
    )
    max_lateral_yaw_rate = bounded_float(
        params.get("maxLateralYawRate", params.get("max_lateral_yaw_rate", params.get("maxLateralCorrection", params.get("max_lateral_correction", 0.15)))),
        0.15,
        0.0,
        0.45,
    )
    ...
    use_route_speed = bool_param(params, ("useRouteSpeed", "use_route_speed"), True)
    ...
    route_transform_json_arg = shlex.quote(str(effective_route_path) + ".relocalize.json") if relocalization.get("enabled") else "''"
    ...
    if relocalization.get("enabled"):
        ...
        xy_search_radius = bounded_float(
            params.get("relocalizeSearchRadius", params.get("relocalize_search_radius", max_route_deviation)),
            max_route_deviation,
            0.5,
            8.0,
        )
        xy_search_step = bounded_float(
            params.get("relocalizeSearchStep", params.get("relocalize_search_step", os.environ.get("GO2_RELOCALIZE_SEARCH_STEP", 2.0))),
            2.0,
            0.5,
            4.0,
        )
        yaw_step = bounded_float(
            params.get("relocalizeYawStepDeg", params.get("relocalize_yaw_step_deg", os.environ.get("GO2_RELOCALIZE_YAW_STEP_DEG", 30.0))),
            30.0,
            5.0,
            60.0,
        )
        fitness_threshold = bounded_float(
            params.get("relocalizeFitnessThreshold", params.get("relocalize_fitness_threshold", os.environ.get("GO2_RELOCALIZE_FITNESS_THRESHOLD", 1.0))),
            1.0,
            0.05,
            5.0,
        )
        relocalize_cmd = (
            ...
            "-p cloud_topic:=/cloud_registered -p odom_topic:=/Odometry "
            "-p collect_seconds:=%s -p xy_search_radius:=%s -p xy_search_step:=%s "
            "-p yaw_search_deg:=180.0 -p yaw_step_deg:=%s -p fitness_threshold:=%s "
            "|| exit $?; "
        ) % (
    ...
    follower_cmd = (
        ...
        "-p route_file:=%s -p route_transform_json:=%s -p v_base:=%s -p max_vx:=%s "
        "-p k_yaw:=%s -p max_yaw_rate:=%s -p lateral_yaw_gain:=%s "
        "-p max_lateral_yaw_rate:=%s -p lateral_deadband:=%s -p lookahead_distance:=%s -p corner_turn_distance:=%s "
        "-p lookahead_straight_distance:=0.700 -p lookahead_corner_distance:=0.250 "
        "-p lookahead_corner_scan_distance:=1.000 -p corner_exit_distance:=0.300 "
        "-p corner_heading_blend:=0.200 -p corner_lateral_yaw_gain_scale:=0.400 "
        "-p reach_distance:=0.4 -p goal_distance:=0.25 "
        "-p loop_mode:=%s -p search_window:=6 -p turn_in_place_angle:=%s "
        "-p slow_down_angle:=%s -p turn_crawl_vx:=0.060 -p stuck_time:=3.0 -p relocalize_distance:=1.5 "
        "-p soft_unstick_yaw_rate:=0.20 -p soft_unstick_min_moved:=0.12 "
    ...
    return ros_env_prefix() + (
        ...
        "touch /tmp/go2_saas_video.run; "
        "setsid nohup python3 -u %s video-loop --seconds 20 --upload --run-file /tmp/go2_saas_video.run "
        ...
        "%s echo SAFE_STARTED log=%s; sleep 1; %s echo FOLLOWER_STARTED log=%s; echo PATROL_STARTED route=%s speed=%s loop=%s log_dir=%s"
    ) % (
        patrol_run_dir_arg,
