...
class UnitreeSafeCmdNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('cloud_recovery_frames', 5)
        self.declare_parameter('startup_enable_file', '')
        ...
        self.cloud_recovery_frames = max(
            1,
            int(self.get_parameter('cloud_recovery_frames').value),
        )
        self.startup_enable_file = str(
            self.get_parameter('startup_enable_file').value
        ).strip()
        ...
        self.cloud_recovering = True
        self.cloud_recovery_count = 0
        self.cloud_timeout_active = False
        self.motion_interlock_released = not self.startup_enable_file
        ...
        self.get_logger().info(
            ...
            f'recovery_frames={self.cloud_recovery_frames}, qos_depth=2/best_effort'
        )
        if self.startup_enable_file:
            self.get_logger().warn(
                'startup motion interlock active; waiting for '
                f'{self.startup_enable_file}'
            )
    ...
    def process_cloud_message(self, msg):
        ...
        if self.cloud_recovering:
            self.cloud_recovery_count += 1
            if self.cloud_recovery_count >= self.cloud_recovery_frames:
                self.cloud_recovering = False
                self.cloud_timeout_active = False
                self.get_logger().info(
                    'cloud stream recovered after '
                    f'{self.cloud_recovery_count} consecutive fresh frames'
                )
        stamp_time = (
    ...
    def publish_safe_cycle(self):
        now = time.time()
        if (
            not self.motion_interlock_released
            and self.startup_enable_file
            and os.path.isfile(self.startup_enable_file)
        ):
            self.motion_interlock_released = True
            self.get_logger().info(
                'startup motion interlock released: '
                f'{self.startup_enable_file}'
            )
        ...
        if cloud_receive_age > self.cloud_timeout:
            if not self.cloud_timeout_active:
                self.cloud_timeout_active = True
                self.cloud_recovering = True
                self.cloud_recovery_count = 0
                self.get_logger().warn(
                    'cloud stream timed out; holding zero speed until '
                    f'{self.cloud_recovery_frames} fresh frames arrive'
                )
        ...
        if not self.motion_interlock_released:
            out_vx, out_vy, out_yaw_rate = (0.0, 0.0, 0.0)
            reason = 'startup_interlock'
        ...
        elif self.cloud_recovering:
            out_vx, out_vy, out_yaw_rate = (0.0, 0.0, 0.0)
            reason = 'cloud_recovery'
