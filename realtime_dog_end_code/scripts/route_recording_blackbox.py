...
def file_has_at_least_lines(path, minimum):
    try:
        with open(path, "r") as handle:
...
def run_observer_warmup_gate(workspace, log_path):
    result = run_command(
        [
            "bash",
            str(Path(workspace) / "scripts/ensure_base_ready.sh"),
            "--patrol-start-gate",
        ],
        timeout=140,
    )
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "observer warmup gate failed rc=%d: %s"
            % (
                result.returncode,
                (result.stderr or result.stdout).strip(),
            )
    ...
    return {
        ...
        "ready": "PATROL_START_GATE_READY" in result.stdout,
...
def copy_base_evidence(
    workspace,
    run_dir,
    offsets=None,
    end_offsets=None,
    output_dir_name="base_logs",
):
    sources = base_evidence_sources(workspace)
    offsets = offsets or {}
    output_dir = Path(run_dir) / output_dir_name
    ...
    for source in sources:
        ...
        try:
            ...
            source_size_at_copy = source.stat().st_size
            end_offset = min(
                source_size_at_copy,
                max(
                    0,
                    int(
                        ...
                        else source_size_at_copy
                    ),
                ),
            )
            truncated_since_start = end_offset < start_offset
        ...
        copied.append(
            {
                ...
                "source_size_at_stop_bytes": end_offset,
                "source_size_at_copy_bytes": source_size_at_copy,
...
def summarize_fastlio_health(path):
    try:
        ...
        return {
            "available": False,
            "recovery_events": 0,
            "health_failed_events": 0,
            "permanent_lock_events": 0,
        }
    recovery_lines = [
        line
        for line in text.splitlines()
        if "FAST_LIO_RECOVERY" in line
    ]
    failed_lines = [
        line
        for line in text.splitlines()
        if re.search(r"FAST_LIO_HEALTH.*FAILED", line)
    ]
    permanent_lines = [
        line
        for line in text.splitlines()
        if re.search(
            r"odometry output is now locked|"
            r"odometry output is locked until base restart",
            line,
            flags=re.IGNORECASE,
        )
    ]
    return {
        "available": True,
        "recovery_events": len(recovery_lines),
        "health_failed_events": len(failed_lines),
        "permanent_lock_events": len(permanent_lines),
        "recovery_examples": recovery_lines[:10],
        "health_failed_examples": failed_lines[:10],
        "permanent_lock_examples": permanent_lines[:10],
    }


def rosbag_topic_timing(
    rosbag_dir,
    start_epoch=None,
    end_epoch=None,
):
    topic_times = {}
    errors = []
    for database in sorted(Path(rosbag_dir).glob("*.db3")):
        try:
            ...
            try:
                ...
                for topic_id, topic_name in topic_rows:
                    ...
                    if start_epoch is not None:
                        ...
                        values.append(int(float(start_epoch) * 1e9))
                    if end_epoch is not None:
                        clauses.append("timestamp <= ?")
                        values.append(int(float(end_epoch) * 1e9))
                    ...
                    topic_times.setdefault(topic_name, []).extend(
                        int(row[0]) for row in rows
                    )
            finally:
                connection.close()
        except (OSError, sqlite3.Error, TypeError, ValueError) as exc:
            errors.append("%s:%r" % (database.name, exc))
    result = {}
    for topic, timestamps in sorted(topic_times.items()):
        ...
        result[topic] = {
            "count": len(ordered),
            "record_time_gap_s": summarize_values(gaps),
        }
    return {
...
def cleanup_started_processes(state):
    ...
    remove_file(state.get("recording_enable_file", ""))
    results = {}
...
def start_recording(args):
    ...
    state = {
        ...
        "recording_enable_file": str(
            run_dir / ".route_recording_enabled"
        ),
        "processes": {},
        "base_log_offsets_pre_observer": capture_log_offsets(workspace),
        "base_log_offsets": {},
    }
    ...
    manifest = {
        ...
        "observer_policy": {
            ...
            "observer_nice": int(OBSERVER_NICE),
            "formal_recording_after_warmup_gate": True,
            "startup_and_formal_base_logs_separated": True,
            "route_writer_enabled_only_after_formal_gate": True,
        },
    }
    atomic_write_json(run_dir / "manifest.json", manifest)

    try:
        session_pre_observer = capture_localization_session(
            workspace,
            run_dir / "localization_session_pre_observer.json",
            run_dir / "localization_session_pre_observer.log",
        )
        manifest["localization_session_pre_observer"] = (
            session_pre_observer
        )
        take_snapshot(
            workspace,
        ...
        remove_file(state["recording_enable_file"])
        ...
        manifest["observer_warmup_gate"] = run_observer_warmup_gate(
            workspace,
            run_dir / "observer_warmup_gate.log",
        )
        ...
        spawn_process(
            ...
            [
                ...
                "-p",
                "startup_enable_file:=%s"
                % state["recording_enable_file"],
            ],
            recorder_log,
            state,
        )
        ...
        manifest["formal_recording_gate"] = run_observer_warmup_gate(
            workspace,
            run_dir / "formal_recording_gate.log",
        )
        manifest["startup_base_logs"] = copy_base_evidence(
            workspace,
            run_dir,
            offsets=state.get(
                "base_log_offsets_pre_observer",
                {},
            ),
            output_dir_name="base_logs_startup",
        )
        state["base_log_offsets"] = capture_log_offsets(workspace)
        state["status"] = "recording"
        state["formal_recording_started_epoch"] = time.time()
        Path(state["recording_enable_file"]).touch()
        if not wait_for(
            lambda: (
                process_group_alive(
                    state["processes"]["route_recorder"]["pgid"]
                ...
                and file_contains(
                    recorder_log,
                    "ROUTE_RECORDER_GATE_RELEASED",
                )
                and route_file.is_file()
                and file_has_at_least_lines(route_file, 2)
            ),
            5.0,
        ):
            raise RuntimeError(
                "route recorder did not release after formal gate"
            )
        state["ready_at"] = iso_time()
        atomic_write_json(str(state_path), state)
        manifest["status"] = "recording"
        manifest["formal_recording_started_epoch"] = state[
            "formal_recording_started_epoch"
        ]
        ...
        manifest["evidence"] = {
            ...
            "observer_warmup_gate": "observer_warmup_gate.log",
            "formal_recording_gate": "formal_recording_gate.log",
            "route_writer_gate": state["recording_enable_file"],
            "startup_base_logs": "base_logs_startup",
            "formal_base_logs": "base_logs",
        }
        atomic_write_json(run_dir / "manifest.json", manifest)
...
def finalize_rosbag(
    ...
    return {
        "available": True,
        "returncode": result.returncode,
        "topic_timing": rosbag_topic_timing(
            rosbag_dir,
...
def stop_recording(args):
    ...
    formal_recording_stopped_epoch = time.time()
    base_log_offsets_at_formal_stop = capture_log_offsets(workspace)
    remove_file(state.get("recording_enable_file", ""))
    remove_file(state.get("active_file", DEFAULT_ACTIVE_FILE))
    processes = state.get("processes", {})
    stop_results = {}
    ...
    rosbag = finalize_rosbag(
        run_dir,
        start_epoch=state.get(
            "formal_recording_started_epoch"
        ),
        end_epoch=formal_recording_stopped_epoch,
    )
    base_logs = copy_base_evidence(
        workspace,
        run_dir,
        offsets=state.get("base_log_offsets", {}),
        end_offsets=base_log_offsets_at_formal_stop,
    )
    fastlio_log = run_dir / "base_logs" / "fast_lio.log"
    fastlio_health = summarize_fastlio_health(fastlio_log)
    if audit is not None:
        audit["fastlio_health"] = fastlio_health
        audit["rosbag_topic_timing"] = rosbag.get(
            "topic_timing",
            {},
        )
        atomic_write_json(
    ...
    route_geometry_report = (
        audit.get("route_geometry", {})
        if audit is not None
        else {}
    )
    if route_geometry_report.get("waypoint_count", 0) < 5:
        errors.append("route_has_fewer_than_5_waypoints")
    if route_geometry_report.get("path_length_m", 0.0) < 1.0:
        errors.append("route_length_below_1m")
    if route_geometry_report.get("spacing_above_1_00m", 0) > 0:
        errors.append("route_contains_waypoint_gap_above_1m")
    if fastlio_health.get("permanent_lock_events", 0) > 0:
        errors.append("fastlio_permanent_lock_during_recording")
    if fastlio_health.get("health_failed_events", 0) > 0:
        errors.append("fastlio_health_failure_during_recording")
    if fastlio_health.get("recovery_events", 0) > 0:
        errors.append("fastlio_recovery_during_recording")
    odom_bag_timing = (
        rosbag.get("topic_timing", {})
        .get("topics", {})
        .get("/Odometry", {})
        .get("record_time_gap_s", {})
    )
    odom_bag_max_gap = odom_bag_timing.get("max")
    if (
        odom_bag_max_gap is not None
        and odom_bag_max_gap > 0.35
    ):
        errors.append(
            "odom_record_gap_during_recording:%.6fs"
            % odom_bag_max_gap
        )
    ...
    sidecar = {
        ...
        "formal_recording_started_epoch": state.get(
            "formal_recording_started_epoch"
        ),
        "formal_recording_stopped_epoch": (
            formal_recording_stopped_epoch
        ),
