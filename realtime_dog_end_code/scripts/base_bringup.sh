...
FASTLIO_FRESH_MAX_AGE_MS=${GO2_FASTLIO_FRESH_MAX_AGE_MS:-500}
...
FASTLIO_FRESH_TIMEOUT=${GO2_FASTLIO_FRESH_TIMEOUT:-120}
...
wait_fastlio_freshness() {
  echo "[base_bringup] requiring ${FASTLIO_FRESH_FRAMES} consecutive fresh FAST-LIO frames..."
  if [ ! -f "$FASTLIO_FRESH_CHECKER" ]; then
    echo "[base_bringup] ERROR: freshness checker missing: ${FASTLIO_FRESH_CHECKER}"
    return 1
  fi

  if python3 -u "$FASTLIO_FRESH_CHECKER" \
      --snapshot "$FASTLIO_SNAPSHOT" \
      --frames "$FASTLIO_FRESH_FRAMES" \
      ...
      --timeout "$FASTLIO_FRESH_TIMEOUT"; then
    echo "[base_bringup] FAST-LIO freshness gate passed."
    return 0
  fi

  echo "[base_bringup] ERROR: FAST-LIO freshness gate failed."
  return 1
}
...
validate_livox_stream() {
  echo "[base_bringup] validating Livox frame timing and point counts..."
  if timeout 12 python3 "${WS}/scripts/check_livox_stream.py" \
      --driver-log "${LOG_DIR}/livox.log" --duration 8.0 --min-reports 3; then
    echo "[base_bringup] Livox stream validation passed."
    return 0
  fi

  echo "[base_bringup] ERROR: Livox stream validation failed."
  return 1
}
...
start_livox_driver() {
  ...
  for attempt in 1 2 3 4 5; do
    ...
    if wait_livox_topics 45 && validate_livox_stream; then
...
if ! wait_fastlio_freshness; then
  exit 21
fi
