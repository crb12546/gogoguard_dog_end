...
def route_recording_evidence(route_path):
    ...
    sidecar = None
    expected_sha256 = ""
    exact_sidecar_exists = sidecar_path.is_file()
    if exact_sidecar_exists:
        try:
    ...
    used_content_alias = False
    if sidecar is None or expected_sha256 != actual_sha256:
        # The SaaS platform may download the exact recorded CSV under a
        # stable plan filename (for example xbf2.csv).  Reuse recording
        # evidence only when the candidate source CSV is still present and
        # byte-identical; never trust a filename or a sidecar hash alone.
        alias_sidecar = None
        alias_metadata = None
        for candidate_sidecar in sorted(
            route_path.parent.glob("*.csv.recording.json")
        ):
            if candidate_sidecar == sidecar_path:
                continue
            candidate_route = Path(
                str(candidate_sidecar)[: -len(".recording.json")]
            )
            if (
                not candidate_route.is_file()
                or sha256_file(candidate_route) != actual_sha256
            ):
                continue
            try:
                candidate = json.loads(candidate_sidecar.read_text())
            except Exception:  # noqa: BLE001
                continue
            if (
                candidate.get("schema") == "go2.route_recording_link.v1"
                and str(candidate.get("route_sha256", ""))
                == actual_sha256
            ):
                alias_sidecar = candidate_sidecar
                alias_metadata = candidate
                break
        if alias_sidecar is not None:
            sidecar_path = alias_sidecar
            sidecar = alias_metadata
            expected_sha256 = actual_sha256
            used_content_alias = True
        elif exact_sidecar_exists:
            raise RuntimeError(
                "ROUTE_RECORDING_LINK_MISMATCH route=%s "
                "expected=%s actual=%s"
                % (
                    route_path,
                    expected_sha256 or "missing",
                    actual_sha256,
                )
            )
        else:
            evidence["reason"] = "recording sidecar not found"
            return evidence
    ...
    evidence.update(
        {
            "available": True,
            "reason": (
                "route hash matches recording blackbox by content alias"
                if used_content_alias
                else "route hash matches recording blackbox"
            ),
            "sidecarPath": str(sidecar_path),
            "contentAlias": used_content_alias,
...
def horizontal_route_evidence(route_path):
    """Resolve a hash-locked gravity-level route beside the source CSV."""
    route_path = Path(route_path)
    horizontal_route_path = Path(str(route_path) + ".horizontal.csv")
    metadata_path = Path(str(route_path) + ".horizontal.json")
    source_sha256 = sha256_file(route_path)
    metadata = None
    exact_files_exist = (
        horizontal_route_path.is_file() and metadata_path.is_file()
    )
    if exact_files_exist:
        try:
            metadata = json.loads(metadata_path.read_text())
        except Exception as exc:  # noqa: BLE001
            raise RuntimeError(
                "horizontal route metadata is unreadable: %s: %s"
                % (metadata_path, exc)
            )
        if metadata.get("schema") != "go2.horizontal_route.v1":
            raise RuntimeError(
                "HORIZONTAL_ROUTE_SCHEMA_INVALID metadata=%s"
                % metadata_path
            )
    ...
    if (
        metadata is None
        or str(metadata.get("source_route_sha256", ""))
        != source_sha256
    ):
        # The SaaS platform may download the exact same route under a stable
        # plan filename while old companion files remain beside it.  Reuse a
        # calibration only by matching its source CSV hash, never by filename
        # or route title.
        alias_metadata_path = None
        alias_route_path = None
        alias_metadata = None
        for candidate_metadata in sorted(
            route_path.parent.glob("*.csv.horizontal.json")
        ):
            if candidate_metadata == metadata_path:
                continue
            candidate_route = candidate_metadata.with_suffix(".csv")
            if not candidate_route.is_file():
                continue
            try:
                candidate = json.loads(candidate_metadata.read_text())
            except Exception:  # noqa: BLE001
                continue
            if (
                candidate.get("schema") == "go2.horizontal_route.v1"
                and str(candidate.get("source_route_sha256", ""))
                == source_sha256
            ):
                alias_metadata_path = candidate_metadata
                alias_route_path = candidate_route
                alias_metadata = candidate
                break
        if alias_metadata_path is not None:
            metadata_path = alias_metadata_path
            horizontal_route_path = alias_route_path
            metadata = alias_metadata
            used_content_alias = True
    evidence = {
        "available": False,
        "sourceRouteSha256": source_sha256,
        "horizontalRoutePath": str(horizontal_route_path),
        "metadataPath": str(metadata_path),
    }
    if not horizontal_route_path.is_file() or not metadata_path.is_file():
        evidence["reason"] = "horizontal route or metadata not found"
        return evidence
    if metadata is None:
        try:
            metadata = json.loads(metadata_path.read_text())
    ...
    expected_source_sha256 = str(
        metadata.get("source_route_sha256", "")
    )
    if (
        not source_sha256
        ...
        raise RuntimeError(
            "HORIZONTAL_ROUTE_SOURCE_MISMATCH route=%s expected=%s actual=%s"
            % (
                route_path,
                expected_source_sha256 or "missing",
                source_sha256 or "missing",
            )
        )
    actual_horizontal_sha256 = sha256_file(horizontal_route_path)
    expected_horizontal_sha256 = str(
        metadata.get("horizontal_route_sha256", "")
    )
    if (
        not actual_horizontal_sha256
        or expected_horizontal_sha256 != actual_horizontal_sha256
    ):
        raise RuntimeError(
            "HORIZONTAL_ROUTE_CONTROL_MISMATCH route=%s expected=%s actual=%s"
            % (
                horizontal_route_path,
                expected_horizontal_sha256 or "missing",
                actual_horizontal_sha256 or "missing",
            )
        )
    line_count = validate_route_csv(horizontal_route_path)
    expected_points = int(metadata.get("route_points", 0))
    if expected_points <= 1 or line_count != expected_points + 1:
        raise RuntimeError(
            "HORIZONTAL_ROUTE_POINT_COUNT_MISMATCH expected=%d lines=%d"
            % (expected_points, line_count)
        )
    evidence.update(
        {
            "available": True,
            "reason": (
                "source and horizontal route hashes match calibrated metadata"
                + (
                    " by content alias"
                    if used_content_alias
                    else ""
            ...
            "contentAlias": used_content_alias,
            "horizontalRouteSha256": actual_horizontal_sha256,
            "routePoints": expected_points,
            "metadata": metadata,
        }
    )
    return evidence
...
def prepare_route_csv(params, dry_run=False):
    ...
    if not dry_run:
        ...
        info["horizontalRouteEvidence"] = horizontal_route_evidence(
            route_path
        )
...
def start_patrol_command(route_path, params, route_info=None):
    ...
    if loop_mode not in ("once", "pingpong"):
        loop_mode = "once"
    use_body_yaw_alignment = bool_param(
        params,
        ("useBodyYawAlignment", "use_body_yaw_alignment"),
        env_bool("GO2_PATROL_BODY_YAW_ALIGNMENT", True),
    )
    body_yaw_topic_value = params.get(
        "bodyYawTopic",
        params.get(
            "body_yaw_topic",
            os.environ.get("GO2_BODY_YAW_TOPIC", "/lf/sportmodestate"),
        ),
    )
    body_yaw_topic = str(
        body_yaw_topic_value
        if body_yaw_topic_value not in (None, "")
        else "/lf/sportmodestate"
    ).strip()
    if (
        not re.match(r"^/[A-Za-z0-9_/]+$", body_yaw_topic)
        or "//" in body_yaw_topic
    ):
        raise RuntimeError(
            "invalid body yaw topic: %s" % body_yaw_topic
        )
    body_yaw_alignment_samples = int(
        round(
            bounded_float(
                params.get(
                    "bodyYawAlignmentSamples",
                    params.get("body_yaw_alignment_samples", 10),
                ),
                10,
                5,
                50,
    ...
    body_yaw_alignment_max_spread_deg = bounded_float(
        params.get(
            "bodyYawAlignmentMaxSpreadDeg",
            params.get("body_yaw_alignment_max_spread_deg", 2.0),
        ),
        2.0,
        0.25,
        5.0,
    )
    body_yaw_max_age = bounded_float(
        params.get(
            "bodyYawMaxAge",
            params.get("body_yaw_max_age", 0.25),
        ),
        0.25,
        0.10,
        0.75,
    ...
    horizontal_evidence = (
        route_info.get("horizontalRouteEvidence")
        if route_info is not None
        else None
    )
    if not isinstance(horizontal_evidence, dict):
        horizontal_evidence = horizontal_route_evidence(route_path)
    use_horizontal_frame = bool_param(
        params,
        ("useHorizontalFrame", "use_horizontal_frame"),
        env_bool(
            "GO2_PATROL_HORIZONTAL_FRAME",
            bool(horizontal_evidence.get("available")),
        ),
    )
    if use_horizontal_frame and not horizontal_evidence.get("available"):
        raise RuntimeError(
            "HORIZONTAL_ROUTE_CALIBRATION_REQUIRED route=%s reason=%s"
            % (
                route_path,
                horizontal_evidence.get("reason", "unavailable"),
            )
        )
    if use_horizontal_frame:
        # A scalar body-yaw replacement would recreate the frame mismatch that
        # this mode removes.  Body IMU remains a startup gravity cross-check.
    ...
    if route_info is not None:
        ...
        route_info["horizontalRouteEvidence"] = horizontal_evidence
        route_info["horizontalFrameEnabled"] = use_horizontal_frame
    ...
    horizontal_route_copy_path = (
        patrol_run_dir / "route_horizontal.csv"
    )
    horizontal_metadata_copy_path = (
        patrol_run_dir / "route_horizontal.json"
    )
    follower_route_path = (
        horizontal_route_copy_path
        if use_horizontal_frame
        else effective_route_path
    )
    follower_route_arg = shlex.quote(str(follower_route_path))
    horizontal_metadata_arg = shlex.quote(
        str(horizontal_metadata_copy_path)
        if use_horizontal_frame
        else ""
    )
    ...
    if route_info is not None:
        ...
        route_info["followerRoutePath"] = str(follower_route_path)
    ...
    use_body_yaw_alignment_arg = (
        "true" if use_body_yaw_alignment else "false"
    )
    use_horizontal_frame_arg = (
        "true" if use_horizontal_frame else "false"
    )
    body_yaw_topic_arg = shlex.quote(body_yaw_topic)
    body_yaw_alignment_samples_arg = shlex.quote(
        str(body_yaw_alignment_samples)
    )
    body_yaw_alignment_max_spread_deg_arg = shlex.quote(
        "%.3f" % body_yaw_alignment_max_spread_deg
    )
    body_yaw_max_age_arg = shlex.quote("%.3f" % body_yaw_max_age)
    ...
    rosbag_cmd = "nice -n 10 ros2 bag record -o %s %s" % (
    ...
    controller_manifest_name = (
        (
            "deployed_go2_2_nearest_lookahead_unified_horizontal_frame"
            if use_horizontal_frame
            else "deployed_go2_2_nearest_lookahead_with_body_yaw_adapter"
        )
        if (use_horizontal_frame or use_body_yaw_alignment)
        else "deployed_go2_2_nearest_lookahead_unchanged"
    )
    heading_feedback_policy = (
        (
            "full_fast_lio_quaternion_in_frozen_gravity_level_frame"
            if use_horizontal_frame
            else "unitree_body_yaw_plus_startup_lio_yaw_offset"
        )
        if (use_horizontal_frame or use_body_yaw_alignment)
        else "raw_fast_lio_euler_yaw"
    )
    horizontal_route_sha256 = (
        horizontal_evidence.get("horizontalRouteSha256", "not_enabled")
        if use_horizontal_frame
        else "not_enabled"
    )
    horizontal_metadata_sha256 = (
        sha256_file(horizontal_evidence.get("metadataPath", ""))
        if use_horizontal_frame
        else "not_enabled"
    )
    manifest_text = (
        ...
        "controller_trace_wrapper_policy=subclass_calls_base_control_with_selected_heading_feedback_after_interlock_release\n"
        "controller_heading_feedback=%s\n"
        "horizontal_frame_enabled=%s\n"
        "horizontal_route_copy=%s\n"
        "horizontal_route_sha256=%s\n"
        "horizontal_metadata_copy=%s\n"
        "horizontal_metadata_sha256=%s\n"
        "horizontal_mapping=full_position_and_quaternion_one_frozen_rigid_rotation\n"
        "horizontal_startup_sources=fastlio_full_quaternion_plus_livox_gravity_plus_unitree_gravity\n"
        "horizontal_runtime_body_yaw_replacement=false\n"
        "body_yaw_alignment_enabled=%s\n"
        "body_yaw_topic=%s\n"
        "body_yaw_qos=keep_last_1_best_effort\n"
        "body_yaw_alignment_samples=%s\n"
        "body_yaw_alignment_max_spread_deg=%s\n"
        "body_yaw_max_age_s=%s\n"
        "body_yaw_stale_policy=zero_velocity_hold_until_fresh\n"
        ...
        "startup_motion_interlock=trace_wrapper_zero_until_post_follower_gate\n"
        "follower_odom_gate=5_fresh_callbacks_before_motion\n"
        "startup_cpu_max_pct=85.0\n"
        "startup_stable_samples=3\n"
        "runtime_safe_node=deployed_source_unchanged\n"
        "runtime_fastlio=recover_positive_gap_without_permanent_latch\n"
        ...
        "observer_process_nice=10\n"
        "observer_warmup_gate=cpu_and_consecutive_fastlio_frames\n"
        ...
        follower_trace_sha256,
        heading_feedback_policy,
        str(use_horizontal_frame).lower(),
        (
            horizontal_route_copy_path.name
            if use_horizontal_frame
            else "not_enabled"
        ),
        horizontal_route_sha256,
        (
            horizontal_metadata_copy_path.name
            if use_horizontal_frame
            else "not_enabled"
        ),
        horizontal_metadata_sha256 or "unavailable",
        str(use_body_yaw_alignment).lower(),
        body_yaw_topic,
    ...
    horizontal_route_copy_cmd = ""
    if use_horizontal_frame:
        horizontal_route_copy_cmd = (
            "cp -- %s %s || exit 5; cp -- %s %s || exit 5;"
            % (
                shlex.quote(
                    horizontal_evidence["horizontalRoutePath"]
                ),
                shlex.quote(str(horizontal_route_copy_path)),
                shlex.quote(horizontal_evidence["metadataPath"]),
                shlex.quote(str(horizontal_metadata_copy_path)),
            )
        )
    ...
    performance_monitor_cmd = (
        "nice -n 10 python3 -u %s --interval 1.0 "
        "--filesystem-path %s"
    ) % (performance_monitor_script, patrol_run_dir_arg)
    experiment_telemetry_cmd = (
        "nice -n 10 python3 -u %s --profile patrol --output %s"
    ) % (
    ...
    horizontal_frame_params = (
        (
            "-p use_horizontal_frame:=true "
            "-p horizontal_frame_metadata:=%s "
            "-p horizontal_imu_topic:=/livox/imu "
            "-p horizontal_frame_samples:=15 "
            "-p horizontal_frame_max_spread_deg:=1.500 "
            "-p horizontal_frame_max_source_disagreement_deg:=3.000 "
            "-p horizontal_frame_max_gyro:=0.080 "
            "-p horizontal_imu_max_age:=0.250"
        )
        % horizontal_metadata_arg
        if use_horizontal_frame
        else "-p use_horizontal_frame:=false"
    )
    follower_cmd = (
        ...
        "-p trace_file:=%s -p motion_enable_file:=%s "
        "-p use_body_yaw_alignment:=%s "
        "-p body_yaw_topic:=%s "
        "-p body_yaw_alignment_samples:=%s "
        "-p body_yaw_alignment_max_spread_deg:=%s "
        "-p body_yaw_max_age:=%s %s"
    ) % (
        ...
        use_body_yaw_alignment_arg,
        body_yaw_topic_arg,
        body_yaw_alignment_samples_arg,
        body_yaw_alignment_max_spread_deg_arg,
        body_yaw_max_age_arg,
        horizontal_frame_params,
    )
    follower_frame_ready_probe = (
        (
            "grep -q 'FOLLOWER_HORIZONTAL_FRAME_READY' %s"
            % shlex.quote(follower_log)
        )
        if use_horizontal_frame
        else (
            "grep -q 'FOLLOWER_BODY_YAW_READY' %s"
            % shlex.quote(follower_log)
            if use_body_yaw_alignment
            else "true"
        )
    )
    session_guard_cmd = (
    ...
    commands = [
        ...
        (
            ...
            "grep -q 'FOLLOWER_ODOM_READY' %s && %s; "
        ...
        (
            "if [ \"$follower_ready\" != 1 ]; then "
                ...
                "startup, did not receive five fresh odometry callbacks, "
                "or did not lock its selected heading frame. "
                ">&2; tail -n 40 %s >&2; %s; exit 43; fi;"
            % (shlex.quote(follower_log), startup_cleanup)
        ),
        "echo FOLLOWER_WARM_READY log=%s;"
        % shlex.quote(follower_log),
        (
            "if ! bash %s --patrol-start-gate; then "
            "echo PATROL_START_GATE_FAILED: CPU, FAST-LIO or follower "
            "startup did not become stable. >&2; %s; exit 45; fi;"
            % (ensure_base_script, startup_cleanup)
        ),
        "touch %s;" % motion_enable_file_arg,
        (
            "motion_released=0; for i in $(seq 1 20); do "
            "if grep -q 'FOLLOWER_MOTION_INTERLOCK_RELEASED' %s; "
            "then motion_released=1; break; fi; sleep 0.1; done;"
            % shlex.quote(follower_log)
        ),
        (
            "if [ \"$motion_released\" != 1 ]; then "
            "echo FOLLOWER_MOTION_RELEASE_FAILED >&2; "
            "tail -n 40 %s >&2; %s; exit 51; fi;"
            % (shlex.quote(follower_log), startup_cleanup)
        ),
        "echo FOLLOWER_STARTED motion_interlock=released log=%s;"
        % shlex.quote(follower_log),
        (
...
def stop_patrol_command():
    ...
    return ros_env_prefix() + (
        ...
        "[ -f \"$run_dir/base_log_offsets.txt\" ]; then "
        ": >\"$run_dir/base_log_stop_offsets.txt\"; "
        "while IFS='|' read -r stop_src ignored_start; do "
        "stop_size=$(stat -c %%s \"$stop_src\" 2>/dev/null || echo 0); "
        "printf '%%s|%%s\\n' \"$stop_src\" \"$stop_size\" "
        ">>\"$run_dir/base_log_stop_offsets.txt\"; "
        "done <\"$run_dir/base_log_offsets.txt\"; fi; "
        ...
        "size=$(stat -c %%s \"$src\" 2>/dev/null || echo 0); "
        "end=$(awk -F'|' -v target=\"$src\" "
        "'$1 == target {print $2; exit}' "
        "\"$run_dir/base_log_stop_offsets.txt\" 2>/dev/null || true); "
        "case \"$end\" in ''|*[!0-9]*) end=\"$size\";; esac; "
        "[ \"$end\" -gt \"$size\" ] && end=\"$size\"; "
        "truncated=false; if [ \"$end\" -lt \"$start\" ]; then "
        "start=0; truncated=true; fi; "
        "count=$((end - start)); "
        "if [ \"$count\" -gt 0 ]; then "
        "tail -c \"+$((start + 1))\" \"$src\" 2>/dev/null | "
        "head -c \"$count\" "
        ">\"$run_dir/base_logs/$(basename \"$src\")\" || true; "
        "else : >\"$run_dir/base_logs/$(basename \"$src\")\"; fi; "
        "printf '%%s|%%s|%%s|%%s\\n' \"$src\" \"$start\" "
        "\"$end\" \"$truncated\" >>\"$run_dir/base_log_slices.txt\"; "
