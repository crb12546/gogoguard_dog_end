...
class RouteQualityTest(unittest.TestCase):
    ...
    def test_straight_line_is_straight_and_isolated_spike_is_removed(self):
        ...
        self.assertEqual(len(clean), 26)
        self.assert_max_gap(clean, 0.405)
    ...
    def test_right_angle_keeps_exact_corner_and_dense_turn_zone(self):
        ...
        self.assertLessEqual(sample_distance(clean[corner_index - 1], clean[corner_index]), 0.305)
        self.assertLessEqual(sample_distance(clean[corner_index], clean[corner_index + 1]), 0.305)
        self.assert_max_gap(clean, 0.405)
    ...
    def test_live_sampler_enters_turn_after_two_confirmations(self):
        ...
        before = len(emitted)
        emitted.extend(sampler.add(RouteSample(1.31, 0.01, math.radians(12.0))))
        self.assertGreater(len(emitted), before)
        self.assertLessEqual(sample_distance(emitted[-2], emitted[-1]), 0.405)
