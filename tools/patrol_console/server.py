...
CAMERA_CAPTURE = f"{WS}/scripts/go2_camera_capture.sh"
CAMERA_PRESET = f"{WS}/scripts/go2_camera_preset.sh"
SAAS_AGENT = f"{WS}/scripts/go2_saas_agent.py"
MANUAL_ANCHOR = f"{WS}/scripts/manual_route_anchor.py"
SESSION_GUARD = f"{WS}/scripts/localization_session_guard.py"
ALIGNMENT_CHECKER = f"{WS}/scripts/check_route_start_alignment.py"
RECORDING_BLACKBOX = f"{WS}/scripts/route_recording_blackbox.py"
SAAS_LOOP_RUN_FILE = "/tmp/go2_saas_loop.run"
CONSOLE_RECORDER_ACTIVE = "/tmp/go2_console_recorder.active"
CONSOLE_RECORDER_SESSION = "/tmp/go2_console_recorder_session.json"
CONSOLE_PCD_ACTIVE = "/tmp/go2_console_pcd.active"
CONSOLE_PCD_SESSION = "/tmp/go2_console_pcd_session.json"
CONSOLE_PATROL_ACTIVE = "/tmp/go2_console_patrol.active"
CONSOLE_PATROL_SESSION = "/tmp/go2_console_patrol_anchor.json"
CONSOLE_PATROL_ROUTE = "/tmp/go2_console_patrol_runtime.csv"
...
ROUTE_NORMAL_SPACING = 0.40
RECORDER_PGIDS = (
    "ps -eo pgid=,comm=,args= | "
    "awk '$2 == \"route_recorder\" || ($2 == \"ros2\" && $0 ~ /[r]oute_recorder/) "
    "{print $1}' | sort -un"
)
...
STATE = {
    ...
    "cellular": {},            # go2-4g-manager 状态与 USB 链路诊断
    "actions_log": [],         # 最近动作结果
}
...
class T(Node):
    def __init__(self):
        ...
        self.create_subscription(LowState, '/lf/lowstate', self.cb_low, latest_qos)
        self.create_subscription(SportModeState, '/lf/sportmodestate', self.cb_sport, latest_qos)
        self.create_subscription(Odometry, '/Odometry', self.cb_odom, latest_qos)
...
while rclpy.ok():
    ...
    # The console renders once per second. Do not consume every high-rate
    # low-state sample and compete with Livox/FAST-LIO on the robot.
    time.sleep(0.05)
    now = time.time()
    if now - last < 1.0:
        continue
...
REMOTE_WRAPPER = (
    f"bash -c '{ENV}; exec nice -n 10 python3 -u -'"
)
...
$1 == "bash" && $0 ~ /bash \/tmp\/([g]o2_camera|[z]1pro)_video_loop\.sh/ { camera_loop=1 }
...
def status_worker():
    while True:
        ...
        cmd += ('; echo __SYS__; cat /sys/class/thermal/thermal_zone0/temp 2>/dev/null || echo 0'
            ...
            f'; camera_source=$(sed -n "s/^GO2_CAMERA_SOURCE=//p" {WS}/config/camera.env 2>/dev/null | tail -1); '
            'echo camera_source=${camera_source:-z1pro}; '
            f'if [ "${{camera_source:-z1pro}}" = unitree_builtin ]; then '
            f'test -x {WS}/bin/go2_builtin_camera_capture && echo camera_builtin_ok || echo camera_builtin_bad; '
            'else '
            ...
            'timeout 1 bash -lc "</dev/tcp/192.168.144.108/2332" >/dev/null 2>&1 && echo z1_gcu_ok || echo z1_gcu_bad; fi'
            '; echo __4G__; cat /run/go2-4g-manager-state.json 2>/dev/null || echo "{}"')
        ...
        if rc in (0, 1):
            ...
            sys_part, _, cellular_part = sys_part.partition("__4G__")
            ...
            for ln in sys_part.strip().splitlines():
                ...
                elif ln in ("camera_builtin_ok", "camera_builtin_bad"):
                    sys_info["camera_builtin"] = ln.endswith("ok")
                elif ln.startswith("camera_source="):
                    sys_info["camera_source"] = ln.split("=", 1)[1]
            cellular = {}
            for ln in reversed(cellular_part.strip().splitlines()):
                try:
...
# ---------------------------------------------------------------- 动作(全部白名单)
def _detached(cmd: str, logname: str, acknowledgement: str = "STARTED") -> str:
    return (
        f"{ENV}; setsid nohup bash -c {shlex.quote(cmd)} </dev/null "
        f">/tmp/console_{logname}.log 2>&1 & echo {acknowledgement}"
    )


def _capture_session(metadata: str) -> str:
    return (
        f"python3 -u {shlex.quote(MANUAL_ANCHOR)} --capture-only "
        f"--metadata {shlex.quote(metadata)} --timeout 12.0 "
        "--stable-seconds 1.0 --min-updates 8 --max-local-gap 0.35 "
...
def _session_guard(
    ...
    active_file: str,
    event_log: str,
    invalidate_output: str = "",
) -> str:
    command = (
        f"python3 -u {shlex.quote(SESSION_GUARD)} "
        f"--metadata {shlex.quote(metadata)} "
        f"--active-file {shlex.quote(active_file)} "
        f"--mode {shlex.quote(mode)} "
        f"--event-log {shlex.quote(event_log)}"
    )
    if invalidate_output:
        command += f" --invalidate-output {shlex.quote(invalidate_output)}"
    return command


def _non_shell_process_probe(pattern: str) -> str:
    """Build a process check that cannot match the enclosing SSH shell."""
    return (
        "ps -eo comm=,args= | "
        "awk '$1 != \"bash\" && $1 != \"sh\" {print}' | "
        f"grep -Eq -- {shlex.quote(pattern)}"
    )


def _base_restart_guard() -> str:
    return (
        ...
        "{print $1}' | "
        'grep -vx "$$" || true); '
        '[ -n "$p" ] && { echo BASE_RESTART_BLOCKED_ACTIVE_MODE; '
...
def act_start_base(_):
    return (
        _base_restart_guard()
        + f"setsid nohup bash {WS}/scripts/base_bringup.sh </dev/null "
...
def act_start_recorder(p):
    ...
    command = (
        f"python3 -u {shlex.quote(RECORDING_BLACKBOX)} start "
        f"--route-file {shlex.quote(path)} "
        f"--min-distance {ROUTE_NORMAL_SPACING:.2f}"
        f"{overwrite_arg}"
    )
    return (
        f"{ENV}; "
...
def act_stop_recorder(_):
    return (
        f"{ENV}; "
        f"python3 -u {shlex.quote(RECORDING_BLACKBOX)} stop"
    )
...
def act_stop_follower(_):
    return (
        f"{ENV}; "
        f"python3 -u {shlex.quote(SAAS_AGENT)} patrol-stop; "
        "rc=$?; [ \"$rc\" -eq 0 ] && echo FOLLOWER_STOPPED; exit \"$rc\""
    )
...
def act_start_follower(p):
    ...
    return (
        f"{ENV}; "
        f"python3 -u {shlex.quote(SAAS_AGENT)} patrol-start "
        f"--route-name {shlex.quote(route_name)} "
        f"--speed {v:.3f} --loop-mode {shlex.quote(loop)} "
        "--localization-mode manual_anchor; "
        "rc=$?; "
        "if [ \"$rc\" -eq 0 ]; then "
        "f=$(readlink -f /tmp/go2_saas_follower.log 2>/dev/null || true); "
        "[ -n \"$f\" ] && ln -sfn \"$f\" /tmp/console_follower.log; "
        "s=$(readlink -f /tmp/go2_saas_safe.log 2>/dev/null || true); "
        "[ -n \"$s\" ] && ln -sfn \"$s\" /tmp/console_safe.log; "
        "fi; exit \"$rc\""
    )
...
def act_stop_all_control(_):
    return (
        f"rm -f {shlex.quote(CONSOLE_PATROL_ACTIVE)}; "
        "pkill -TERM -f "
        "'[l]ocalization_session_guard.py.*go2_console_patrol' "
        "2>/dev/null || true; "
...
def act_start_pcd(p):
    ...
    return mode_guard + (
        ...
        + f"rm -f {shlex.quote(CONSOLE_PCD_ACTIVE)} "
        + f"{shlex.quote(CONSOLE_PCD_SESSION)}; "
        + _capture_session(CONSOLE_PCD_SESSION)
        + " || { echo PCD_SESSION_CAPTURE_FAILED; exit 6; }; "
        ...
        + "; sleep 0.5; "
        + "if ! "
        + _non_shell_process_probe(
            r"[l]ocalization_session_guard.py.*--mode pcd"
        )
        + "; then "
        + f"rm -f {shlex.quote(CONSOLE_PCD_ACTIVE)}; "
        + "echo SESSION_GUARD_NOT_READY; exit 7; fi; "
        + _detached(pcd_inner, "pcd")
        + "; "
        + "echo PCD_CAPTURE_STARTED"
    )
...
def act_stop_pcd(_):
    ...
    return (
        f"rm -f {shlex.quote(CONSOLE_PCD_ACTIVE)}; "
        "pkill -TERM -f "
        "'[l]ocalization_session_guard.py.*--mode pcd' "
        "2>/dev/null || true; "
        'pkill -TERM -f "[g]o2map_capture" '
        "&& echo PCD_SAVING || echo NOT_RUNNING"
    )


def act_camera_probe(_):
    return (
        f'echo __CAPTURE__; {CAMERA_CAPTURE} probe 2>&1 | head -8; '
        f'echo __CONTROL__; {CAMERA_PRESET} probe 2>&1 | head -8'
    )
...
def act_camera_preset(p):
    ...
    return f'{CAMERA_PRESET} {preset}'


def act_camera_snapshot(_):
    return f'{CAMERA_CAPTURE} snapshot | tail -1 | sed "s/^/SNAPSHOT /"'
...
def act_camera_record(p):
    ...
    return f'{CAMERA_CAPTURE} record {seconds} | tail -1 | sed "s/^/VIDEO /"'
...
def act_camera_start_loop(p):
    ...
    return mode_guard + (
        'p=$(ps -eo pid=,comm=,args= | awk \'$2 == "bash" && $0 ~ /^ *[0-9]+ +bash +\/tmp\/[g]o2_camera_video_loop\.sh/ {print $1}\' | grep -vx "$$" || true); '
        '[ -n "$p" ] && { echo CAMERA_LOOP_ALREADY_RUNNING; exit 0; }; '
        'rm -f /tmp/go2_camera_video_loop.run; '
        'cat > /tmp/go2_camera_video_loop.sh <<\'SH\'\n'
        '#!/usr/bin/env bash\n'
        f'touch /tmp/go2_camera_video_loop.run\n'
        'while [ -f /tmp/go2_camera_video_loop.run ]; do\n'
        f'  {CAMERA_CAPTURE} record {seconds} || true\n'
        '  sleep 1\n'
        'done\n'
        'SH\n'
        'chmod +x /tmp/go2_camera_video_loop.sh; '
        'setsid nohup bash /tmp/go2_camera_video_loop.sh </dev/null >/tmp/console_go2_camera_loop.log 2>&1 & echo CAMERA_LOOP_STARTED'
    )


def act_camera_stop_loop(_):
    return (
        'rm -f /tmp/go2_camera_video_loop.run /tmp/z1pro_video_loop.run; '
        'pkill -TERM -f "[g]o2_camera_video_loop.sh|[z]1pro_video_loop.sh" 2>/dev/null || true; '
        'pkill -TERM -f "[g]o2_builtin_camera_capture stream" 2>/dev/null || true; '
...
def api_camera_files():
    ...
    cmd = (
        ...
        r"\( -name '*.mp4' -o -name '*.jpg' -o -name '*.jpeg' \) "
        r"! -name '.*' -size +0c "
