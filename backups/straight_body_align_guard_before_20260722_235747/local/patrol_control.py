...
def lateral_velocity_alignment_allowed(
    control_mode,
    steering_error,
    route_heading_error,
    steering_limit_deg=8.0,
    route_heading_limit_deg=25.0,
):
    """Allow body lateral correction only on an aligned straight.

    Pure-pursuit yaw and ``vy`` both remove cross-track error. Applying both
    while the robot is already steering caused the measured alternating
    overshoot. Curves and corners are yaw-only; FOLLOW may use ``vy`` only
    after both heading signals are within their alignment limits.
    """
    if str(control_mode).upper() != 'FOLLOW':
        return False

    steering_limit = math.radians(max(0.0, float(steering_limit_deg)))
    route_heading_limit = math.radians(
        max(0.0, float(route_heading_limit_deg))
    )
    return (
        abs(normalize_angle(float(steering_error))) <= steering_limit
        and abs(normalize_angle(float(route_heading_error)))
        <= route_heading_limit
    )


def lateral_drift_compensation_target(
    lateral_error,
    base_vy,
    actual_cross_velocity,
    stable_elapsed,
    eligible=True,
    error_release=0.05,
    stable_time=1.20,
    return_gain=0.25,
    max_return_speed=0.035,
    velocity_gain=1.0,
    max_extra_vy=0.04,
):
    """Return a bounded extra ``vy`` for persistent straight-line side slip."""
    # The position controller can spend its whole command merely keeping a
    # crabbed robot parallel to the route. Compare measured route-normal speed
    # with a small desired return speed and add only the missing amount. The
    # caller makes this eligible only on a stable FOLLOW straight, so no bias
    # can carry into a corner, curve, stop or heading recovery.
    lateral_error = float(lateral_error)
    base_vy = float(base_vy)
    actual_cross_velocity = float(actual_cross_velocity)
    error_release = max(0.0, float(error_release))
    stable_time = max(0.0, float(stable_time))
    max_return_speed = max(0.0, float(max_return_speed))
    max_extra_vy = max(0.0, float(max_extra_vy))

    result = {
        'extra_vy': 0.0,
        'closing_speed': 0.0,
        'desired_closing_speed': 0.0,
        'active': False,
    }
    if (
        abs(lateral_error) <= error_release
        or abs(base_vy) <= 1e-6
        or max_return_speed <= 0.0
        or max_extra_vy <= 0.0
    ):
        return result

    error_sign = 1.0 if lateral_error > 0.0 else -1.0
    closing_speed = -error_sign * actual_cross_velocity
    desired_closing_speed = min(
        max_return_speed,
        max(0.0, float(return_gain))
        * max(0.0, abs(lateral_error) - error_release),
    )
    result.update({
        'closing_speed': closing_speed,
        'desired_closing_speed': desired_closing_speed,
    })
    if not bool(eligible) or float(stable_elapsed) < stable_time:
        return result

    missing_closing_speed = max(0.0, desired_closing_speed - closing_speed)
    extra_magnitude = min(
        max_extra_vy,
        max(0.0, float(velocity_gain)) * missing_closing_speed,
    )
    correction_sign = 1.0 if base_vy > 0.0 else -1.0
    result.update({
        'extra_vy': correction_sign * extra_magnitude,
        'active': extra_magnitude > 1e-6,
    })
    return result
