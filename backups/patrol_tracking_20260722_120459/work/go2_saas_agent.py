...
def route_relocalization_plan(route_path, params):
    ...
    required = bool_param(
        ...
        env_bool("GO2_PATROL_REQUIRE_RELOCALIZE", True),
    )
    plan = {
    ...
    if not enabled:
        if not env_bool("GO2_ALLOW_UNTRANSFORMED_PATROL", False):
            raise RuntimeError(
                "untransformed patrol is blocked; enable GO2_ALLOW_UNTRANSFORMED_PATROL only after verifying "
                "the route and current odometry share the same frame"
            )
        plan["reason"] = "explicitly disabled; use only when route and current odometry share one verified frame"
        return plan
...
def start_patrol_command(route_path, params, route_info=None):
    ...
    follower_cmd = (
        ...
        "-p lateral_velocity_gain:=0.400 -p max_vy:=0.100 "
