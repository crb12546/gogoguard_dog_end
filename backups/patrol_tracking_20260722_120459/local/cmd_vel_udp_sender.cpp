...
public:
  CmdVelUdpSender() : Node("cmd_vel_udp_sender")
  {
    ...
    // ROS uses +Y to the robot's left.  The Unitree UDP motion boundary on
    // this robot uses the opposite lateral sign, so convert exactly once here.
    ...
    RCLCPP_INFO(
      this->get_logger(), "UDP sender to %s:%d, max_vy=%.3f, unitree_vy_sign=%+.0f",
      target_ip_.c_str(), target_port_, max_vy_, unitree_vy_sign_);
  }
...
private:
  ...
  void callback(const geometry_msgs::msg::Twist::SharedPtr msg)
  {
    ...
    pkt.vy = unitree_vy_sign_ * limit(msg->linear.y, max_vy_);
