...
def status_worker():
    while True:
        ...
        if rc in (0, 1):
            ...
            wc_part, _, rest2 = rest.partition("__PCDMARK__")
            pcd_part, _, rest3 = rest2.partition("__FOLLOWER__")
...
def act_start_recorder(p):
    ...
    inner = (
        ...
        f"-p route_file:={path} -p min_distance:=0.4"
    )
    return dup_guard + guard + _detached(inner, "recorder")
...
def act_stop_recorder(_):
    ...
    return ('pkill -TERM -f "[r]oute_recorder" && { sleep 1; pkill -KILL -f "[r]oute_recorder" 2>/dev/null; echo STOPPED; } '
            '|| echo NOT_RUNNING') + wc


def act_start_safe(p):
    return "echo SAFE_MANAGED_BY_START_FOLLOWER use_start_follower_for_full_saas_patrol_chain"
...
def act_start_follower(p):
    ...
    params_json = json.dumps({"speed": v, "loopMode": loop}, separators=(",", ":"))
    script = f"""
cd {shlex.quote(WS)}
{ENV}
python3 - <<'PY'
...
agent_path = Path({SAAS_AGENT!r})
route_path = Path({path!r})
params = json.loads({params_json!r})
spec = importlib.util.spec_from_file_location('go2_saas_agent_console', agent_path)
agent = importlib.util.module_from_spec(spec)
spec.loader.exec_module(agent)
route_info = {{}}
cmd = agent.start_patrol_command(route_path, params, route_info=route_info)
print('CONSOLE_START_PATROL_USING_SAAS_AGENT route=%s speed=%s loop=%s' % (route_path.name, params.get('speed'), params.get('loopMode')), flush=True)
sys.exit(subprocess.call(cmd, shell=True, executable='/bin/bash'))
PY
""".strip()
    return route_check + script
...
def api_camera_files():
    ...
    if not host:
        return JSONResponse({"error": "SSH 不可达"}, status_code=503)
    cmd = (
        f'for f in {VIDEO_DIR}/z1pro*.mp4 {VIDEO_DIR}/z1pro*.jpg {VIDEO_DIR}/z1pro*.jpeg; do '
        f'[ -f "$f" ] || continue; '
        f'printf "%s|%s|%s\\n" "$f" "$(stat -c %s "$f")" "$(stat -c %Y "$f")"; done | sort -t "|" -k3,3nr | head -60'
    )
...
def api_action(req: ActionReq):
    ...
    timeout = 160 if req.name == "saas_video_segment" else 120 if req.name == "start_follower" else 90 if req.name in {"camera_record", "camera_start_loop", "camera_snapshot", "saas_heartbeat", "saas_manifest", "saas_command_result"} else 25
