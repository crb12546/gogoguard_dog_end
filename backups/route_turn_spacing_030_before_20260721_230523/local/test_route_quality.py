...
class RouteQualityTest(unittest.TestCase):
    ...
    def test_right_angle_keeps_exact_corner_and_dense_turn_zone(self):
        ...
        self.assertLessEqual(sample_distance(clean[corner_index - 1], clean[corner_index]), 0.155)
        self.assertLessEqual(sample_distance(clean[corner_index], clean[corner_index + 1]), 0.155)
        self.assert_max_gap(clean, 0.405)
    ...
    def test_live_sampler_enters_turn_after_two_confirmations(self):
        ...
        before = len(emitted)
        emitted.extend(sampler.add(RouteSample(1.16, 0.01, math.radians(12.0))))
