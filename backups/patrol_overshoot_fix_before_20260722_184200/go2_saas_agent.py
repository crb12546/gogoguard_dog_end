...
def localization_mode_from_params(params):
    ...
    A route recorded in the current FAST-LIO frame should be started from its
    first waypoint. PCD/ICP is retained only as an explicit recovery operation
    for a route that must be transformed into a different current frame.
    """
    ...
    for key in mode_keys:
        ...
        if normalized not in aliases:
            raise ValueError(
                "invalid localization mode %r; use origin or pcd" % value
            )
    ...
    if requested_modes:
        ...
        if legacy_values:
            ...
            mode = "pcd" if legacy_values[0] else "origin"
        else:
            configured = os.environ.get("GO2_PATROL_LOCALIZATION_MODE", "origin")
            mode = str(configured).strip().lower().replace("-", "_") or "origin"
            if mode in ("direct", "origin_direct", "none"):
                mode = "origin"
            elif mode in ("relocalize", "relocalization", "map"):
                mode = "pcd"
            if mode not in ("origin", "pcd"):
                raise ValueError(
                    "invalid GO2_PATROL_LOCALIZATION_MODE=%r; use origin or pcd"
                    % configured
                )
...
def route_relocalization_plan(route_path, params):
    ...
    if mode == "origin":
        plan["reason"] = "origin_direct: require the current odometry pose to match route waypoint 0"
        return plan
...
def start_patrol_command(route_path, params, route_info=None):
    ...
    max_route_deviation = bounded_float(
        params.get("maxRouteDeviation", params.get("max_route_deviation", 5.0)),
        5.0,
        0.8,
        8.0,
    )
    recovery_distance = bounded_float(
        params.get("recoveryDistance", params.get("recovery_distance", 1.0)),
        1.0,
        0.3,
        2.0,
    )
    recovery_heading_distance = bounded_float(
        params.get("recoveryHeadingDistance", params.get("recovery_heading_distance", 0.35)),
        0.35,
        0.0,
        2.0,
    )
    recovery_yaw_error = bounded_float(
        params.get("recoveryYawError", params.get("recovery_yaw_error", 0.75)),
        0.75,
        0.0,
        3.14,
    )
    recovery_vx = bounded_float(
        params.get("recoveryVx", params.get("recovery_vx", 0.25)),
        0.25,
        0.03,
        0.5,
    )
    max_yaw_rate = bounded_float(
        params.get("maxYawRate", params.get("max_yaw_rate", 0.50)),
        0.50,
        0.15,
        0.8,
    )
    max_non_corner_yaw_rate = bounded_float(
        params.get(
            "maxNonCornerYawRate",
            params.get("max_non_corner_yaw_rate", 0.30),
        ),
        0.30,
        0.10,
        0.50,
    ...
    k_yaw = bounded_float(
        params.get("kYaw", params.get("k_yaw", 0.65)),
        0.65,
        0.10,
        1.20,
    )
    lookahead_distance = bounded_float(
        params.get("lookaheadDistance", params.get("lookahead_distance", 1.30)),
        1.30,
        0.25,
        1.8,
    )
    corner_turn_distance = bounded_float(
        params.get("cornerTurnDistance", params.get("corner_turn_distance", 0.90)),
        0.90,
        0.20,
        2.00,
    )
    turn_in_place_angle = bounded_float(
        params.get("turnInPlaceAngle", params.get("turn_in_place_angle", 1.75)),
        1.75,
        0.6,
        2.4,
    )
    slow_down_angle = bounded_float(
        params.get("slowDownAngle", params.get("slow_down_angle", 0.45)),
        0.45,
        0.2,
        1.5,
    )
    lateral_yaw_gain = bounded_float(
        params.get("lateralYawGain", params.get("lateral_yaw_gain", params.get("lateralCorrectionGain", params.get("lateral_correction_gain", 0.06)))),
        0.06,
        0.0,
        1.0,
    )
    max_lateral_yaw_rate = bounded_float(
        params.get("maxLateralYawRate", params.get("max_lateral_yaw_rate", params.get("maxLateralCorrection", params.get("max_lateral_correction", 0.05)))),
        0.05,
        0.0,
        0.45,
    )
    lateral_deadband = bounded_float(
        params.get("lateralDeadband", params.get("lateral_deadband", 0.03)),
        0.03,
        0.0,
        0.30,
    )
    ...
    effective_route_path = Path(relocalization["runtimeRoutePath"]) if relocalization.get("enabled") else Path(route_path)

    route_arg = shlex.quote(str(effective_route_path))
    source_route_arg = shlex.quote(str(route_path))
    follower_route_arg = source_route_arg if relocalization.get("enabled") else route_arg
    route_transform_param = ""
    if relocalization.get("enabled"):
    ...
    if route_info is not None:
        ...
        route_info["followerRoutePath"] = str(route_path)
        route_info["routeTransformEnabled"] = bool(relocalization.get("enabled"))
    ...
    max_route_deviation_arg = shlex.quote("%.3f" % max_route_deviation)
    recovery_distance_arg = shlex.quote("%.3f" % recovery_distance)
    recovery_heading_distance_arg = shlex.quote("%.3f" % recovery_heading_distance)
    recovery_yaw_error_arg = shlex.quote("%.3f" % recovery_yaw_error)
    recovery_vx_arg = shlex.quote("%.3f" % recovery_vx)
    ...
    lookahead_distance_arg = shlex.quote("%.3f" % lookahead_distance)
    corner_turn_distance_arg = shlex.quote("%.3f" % corner_turn_distance)
    turn_in_place_angle_arg = shlex.quote("%.3f" % turn_in_place_angle)
    slow_down_angle_arg = shlex.quote("%.3f" % slow_down_angle)
    lateral_yaw_gain_arg = shlex.quote("%.3f" % lateral_yaw_gain)
    max_lateral_yaw_rate_arg = shlex.quote("%.3f" % max_lateral_yaw_rate)
    lateral_deadband_arg = shlex.quote("%.3f" % lateral_deadband)
    ...
    route_log_name = re.sub(r"[^A-Za-z0-9_.-]+", "_", route_path.stem).strip("._-") or "route"
    route_log_name = route_log_name[:80]
    patrol_run_dir = PATROL_RUNS_DIR / ("%s_%s" % (time.strftime("%Y%m%d_%H%M%S"), route_log_name))
    ...
    sdk_receiver_cmd = "GO2_SDK_MAX_VY=0.150 %s/build/go2_cmd_vel_bridge/go2_sdk2_udp_receiver %s 5005" % (shlex.quote(WS), sdk_if_arg)
    cmd_vel_sender_cmd = (
        "ros2 run go2_cmd_vel_bridge cmd_vel_udp_sender --ros-args "
        "-p target_ip:=127.0.0.1 -p target_port:=5005 "
        "-p max_vx:=%s -p max_vy:=0.150 -p max_vyaw:=%s "
        "-p unitree_vy_sign:=1.0"
    ) % (speed_arg, max_yaw_rate_arg)
    safe_cmd = (
        "ros2 run go2_fastlio_patrol unitree_safe_cmd_node --ros-args "
        "-p max_vx:=%s -p max_vy:=0.150 -p max_yaw_rate:=%s -p publish_rate:=20.0 "
    ...
    follower_cmd = (
        ...
        "-p route_file:=%s %s-p v_base:=%s -p max_vx:=%s "
        "-p lateral_velocity_gain:=0.750 -p max_vy:=0.150 "
        "-p lateral_velocity_deadband:=0.030 -p lateral_velocity_heading_limit_deg:=25.0 "
        "-p lateral_velocity_pid_enabled:=true "
        "-p lateral_velocity_ki:=0.030 -p lateral_velocity_kd:=0.000 "
        "-p lateral_velocity_integral_vy_limit:=0.030 "
        "-p lateral_velocity_derivative_vy_limit:=0.000 "
        "-p lateral_velocity_derivative_filter_tau:=0.350 "
        "-p lateral_velocity_integral_reset_error:=0.060 "
        "-p lateral_velocity_integral_progress_timeout:=0.750 "
        "-p lateral_velocity_integral_heading_limit_deg:=10.000 "
        "-p lateral_velocity_pid_follow_warmup:=5.000 "
        "-p lateral_velocity_pid_follow_warmup_distance:=2.000 "
        "-p lateral_velocity_pid_warmup_max_vy:=0.150 "
        "-p lateral_velocity_yaw_slow_rate:=0.120 "
        "-p lateral_velocity_yaw_stop_rate:=0.250 "
        "-p max_vy_decel:=0.600 "
        "-p k_yaw:=%s -p max_yaw_rate:=%s -p max_non_corner_yaw_rate:=%s -p lateral_yaw_gain:=%s "
        "-p max_lateral_yaw_rate:=%s -p lateral_deadband:=%s -p lookahead_distance:=%s -p corner_turn_distance:=%s "
        "-p lookahead_straight_distance:=1.450 -p lookahead_corner_distance:=0.350 "
        "-p lookahead_curve_distance:=0.850 -p lookahead_curve_angle_deg:=20.0 "
        "-p lookahead_curve_scan_distance:=4.000 -p curve_heading_preview_distance:=1.200 "
        "-p curve_heading_blend:=0.350 -p curve_yaw_gain_scale:=1.000 "
        "-p curve_lateral_taper_start_distance:=2.000 "
        "-p curve_lateral_zero_distance:=0.800 "
        "-p lookahead_corner_angle_deg:=35.0 "
        "-p lookahead_corner_scan_distance:=2.200 -p corner_exit_distance:=0.900 "
        "-p corner_wiggle_distance:=1.600 -p corner_wiggle_min_opposite_angle_deg:=30.0 "
        "-p corner_heading_blend:=0.300 -p corner_heading_conflict_blend:=0.650 "
        "-p corner_lateral_yaw_gain_scale:=0.300 "
        "-p reach_distance:=0.4 -p goal_distance:=0.25 "
        "-p loop_mode:=%s -p search_window:=6 -p turn_in_place_angle:=%s "
        "-p slow_down_angle:=%s -p turn_crawl_vx:=0.250 -p stuck_time:=2.0 -p relocalize_distance:=1.5 "
        "-p soft_unstick_yaw_rate:=0.00 -p soft_unstick_min_moved:=0.15 "
        "-p progress_s_epsilon:=0.08 -p progress_backtrack_tolerance:=0.12 "
        "-p progress_forward_tolerance:=0.45 -p projection_speed_scale:=1.20 "
        "-p projection_step_margin:=0.010 -p projection_min_step:=0.015 -p projection_max_step:=0.120 "
        "-p start_policy:=route_start -p start_max_distance:=%s -p start_max_yaw_error:=%s "
        "-p allow_global_relocalize:=false -p stuck_relocalize:=false "
        "-p projection_fail_limit:=3 -p stop_on_route_deviation:=true -p recovery_distance:=%s "
        "-p recovery_heading_distance:=%s -p recovery_yaw_error:=%s -p recovery_vx:=%s "
        "-p max_route_deviation:=%s -p use_route_speed:=%s "
        "-p yaw_deadband:=0.08 -p max_vx_accel:=0.60 -p max_yaw_accel:=1.00 "
        "-p yaw_wrap_hysteresis:=0.35 -p max_odom_jump_distance:=0.80 "
        "-p max_odom_jump_z:=0.50 -p max_odom_jump_dt:=2.00"
    ) % (
        follower_route_arg,
        route_transform_param,
        speed_arg,
        ...
        k_yaw_arg,
        max_yaw_rate_arg,
        max_non_corner_yaw_rate_arg,
        lateral_yaw_gain_arg,
        max_lateral_yaw_rate_arg,
        lateral_deadband_arg,
        lookahead_distance_arg,
        corner_turn_distance_arg,
        loop_arg,
        turn_in_place_angle_arg,
        slow_down_angle_arg,
        start_max_distance_arg,
        start_max_yaw_error_arg,
        recovery_distance_arg,
        recovery_heading_distance_arg,
        recovery_yaw_error_arg,
        recovery_vx_arg,
        max_route_deviation_arg,
        use_route_speed_arg,
    )
    return ros_env_prefix() + (
        "conflict=$(ps -eo pid=,comm=,args= | awk '$2 != \"bash\" && $2 != \"sh\" && $0 ~ /[r]oute_recorder|[g]o2map_capture|\\/tmp\\/[z]1pro_video_loop\\.sh/ {print $1}' | grep -vx \"$$\" || true); "
        "[ -n \"$conflict\" ] && { echo PATROL_MODE_CONFLICT recorder_or_pcd_or_manual_video; echo \"$conflict\"; exit 4; }; "
        "p=$(ps -eo pid=,comm=,args= | awk '$2 != \"bash\" && $2 != \"sh\" && $0 ~ /[w]aypoint_follower|[u]nitree_safe_cmd_node|[c]md_vel_udp_sender|[g]o2_sdk2_udp_receiver/ {print $1}' | grep -vx \"$$\" || true); "
        ...
        "ln -sfn %s /tmp/go2_saas_safe.log; "
        "ln -sfn %s /tmp/go2_saas_follower.log; "
        "echo PATROL_LOG_DIR=%s; "
        "bash %s || exit $?; "
        "if ! pgrep -f '[g]o2_base_health_watchdog.py' >/dev/null 2>&1; then "
        "setsid nohup python3 -u %s </dev/null >>%s 2>&1 & echo BASE_HEALTH_WATCHDOG_STARTED; fi; "
        "%s"
        "python3 %s --route-file %s --max-distance %s --max-yaw-error %s || exit $?; "
        "mkdir -p %s; touch %s; "
        "if ! pgrep -f '[g]o2_saas_agent.py video-loop' >/dev/null 2>&1; then "
        "touch %s; "
        "setsid nohup python3 -u %s video-loop --seconds 20 --upload --run-file %s "
        "</dev/null >/tmp/go2_saas_video_loop.log 2>&1 & echo VIDEO_LOOP_STARTED; fi; "
        "%s echo SDK_RECEIVER_STARTED iface=%s log=%s; sleep 4; "
        "if grep -Eq 'StandUp ret=[1-9][0-9]*|BalanceStand ret=[1-9][0-9]*' %s; then "
        "echo MOTION_SDK_NOT_READY: StandUp/BalanceStand returned nonzero. Check GO2_SDK_IF or robot sport mode. >&2; "
        "pkill -TERM -f '[g]o2_sdk2_udp_receiver' 2>/dev/null || true; exit 42; fi; "
        "if ! pgrep -f '[g]o2_sdk2_udp_receiver' >/dev/null 2>&1; then "
        "echo MOTION_SDK_NOT_READY: SDK receiver exited before patrol start. >&2; exit 42; fi; "
        "%s echo CMD_VEL_SENDER_STARTED log=%s; sleep 1; "
        "%s echo SAFE_STARTED log=%s; sleep 1; %s "
        "follower_ready=0; for i in $(seq 1 30); do "
        "if grep -q 'simple csv waypoint_follower started' %s; then follower_ready=1; break; fi; sleep 0.5; done; "
        "if [ \"$follower_ready\" != 1 ]; then "
        "echo FOLLOWER_NOT_READY: waypoint_follower exited during startup. >&2; tail -n 40 %s >&2; "
        "pkill -TERM -f '[w]aypoint_follower|[u]nitree_safe_cmd_node|[c]md_vel_udp_sender|[g]o2_sdk2_udp_receiver' 2>/dev/null || true; exit 43; fi; "
        "echo FOLLOWER_STARTED log=%s; echo PATROL_STARTED route=%s speed=%s loop=%s log_dir=%s"
    ) % (
        patrol_run_dir_arg,
        shlex.quote(now_text()),
...
def stop_patrol_command():
    return ros_env_prefix() + (
        ...
        "p=$(ps -eo pid=,comm=,args= | awk '$2 != \"bash\" && $2 != \"sh\" && $0 ~ /[w]aypoint_follower|[u]nitree_safe_cmd_node|[u]nitree_cmd_node|[c]md_vel_udp_sender|[g]o2_sdk2_udp_receiver/ {print $1}' | grep -vx \"$$\" | tr \"\\n\" \" \" || true); "
        "[ -n \"$p\" ] && kill -TERM $p 2>/dev/null || true; sleep 1; "
        "p2=$(ps -eo pid=,comm=,args= | awk '$2 != \"bash\" && $2 != \"sh\" && $0 ~ /[w]aypoint_follower|[u]nitree_safe_cmd_node|[u]nitree_cmd_node|[c]md_vel_udp_sender|[g]o2_sdk2_udp_receiver/ {print $1}' | grep -vx \"$$\" | tr \"\\n\" \" \" || true); "
        ...
        "echo PATROL_STOPPED"
    ) % (shlex.quote(str(PATROL_VIDEO_ACTIVE_FILE)), shlex.quote(WS))
