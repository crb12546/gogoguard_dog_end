...
class PatrolControlTest(unittest.TestCase):
    ...
    def test_pid_warmup_uses_real_forward_progress(self):
        self.assertFalse(lateral_pid_warmup_ready(
            current_s=10.0,
            start_s=10.0,
            direction=1,
            warmup_distance=2.0,
        ...
        self.assertTrue(lateral_pid_warmup_ready(
            current_s=12.0,
            start_s=10.0,
            direction=1,
            warmup_distance=2.0,
    ...
    def test_pid_warmup_handles_reverse_patrol_direction(self):
        self.assertTrue(lateral_pid_warmup_ready(
            current_s=8.0,
            start_s=10.0,
            direction=-1,
    ...
    def test_pid_warmup_rejects_stale_progress(self):
        self.assertFalse(lateral_pid_warmup_ready(
            current_s=14.0,
            start_s=10.0,
            direction=1,
    ...
    def test_lateral_velocity_yaw_limit_prioritizes_heading(self):
        self.assertAlmostEqual(
            lateral_velocity_yaw_limit(0.15, 0.10, 0.12, 0.25),
            0.15,
        )
        self.assertAlmostEqual(
            lateral_velocity_yaw_limit(0.15, 0.185, 0.12, 0.25),
            0.075,
        )
        self.assertEqual(
            lateral_velocity_yaw_limit(0.15, 0.25, 0.12, 0.25),
            0.0,
        )

    def test_curve_lateral_taper_preserves_then_removes_entry_vy(self):
        self.assertEqual(curve_lateral_taper_scale(3.0, 2.0, 0.8), 1.0)
        self.assertEqual(curve_lateral_taper_scale(2.0, 2.0, 0.8), 1.0)
        self.assertAlmostEqual(
            curve_lateral_taper_scale(1.4, 2.0, 0.8),
            0.5,
        )
        self.assertEqual(curve_lateral_taper_scale(0.8, 2.0, 0.8), 0.0)
        self.assertEqual(curve_lateral_taper_scale(-1.0, 2.0, 0.8), 0.0)

    def test_lateral_velocity_brakes_faster_than_it_accelerates(self):
        accelerated = smooth_lateral_velocity(0.15, 0.0, 0.20, 0.60, 0.05)
        braked = smooth_lateral_velocity(0.0, 0.15, 0.20, 0.60, 0.05)

        self.assertAlmostEqual(accelerated, 0.01)
        self.assertAlmostEqual(braked, 0.12)

    def test_lateral_velocity_does_not_reverse_before_reaching_zero(self):
        command = smooth_lateral_velocity(-0.15, 0.02, 0.20, 0.60, 0.05)
