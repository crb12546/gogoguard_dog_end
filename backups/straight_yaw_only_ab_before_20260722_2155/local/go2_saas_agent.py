...
def start_patrol_command(route_path, params, route_info=None):
    ...
    follower_cmd = (
        ...
        "-p lateral_velocity_steering_limit_deg:=8.0 "
        "-p motion_drift_comp_enabled:=true -p motion_drift_window_sec:=1.000 "
        "-p motion_drift_min_window_sec:=0.600 -p motion_drift_min_along_speed:=0.200 "
        "-p motion_drift_max_route_angle_deg:=6.000 -p motion_drift_min_crab_angle_deg:=5.000 "
        "-p motion_drift_error_activate:=0.080 -p motion_drift_error_release:=0.050 "
        "-p motion_drift_stable_time:=1.200 -p motion_drift_return_gain:=0.250 "
        "-p motion_drift_max_return_speed:=0.035 -p motion_drift_velocity_gain:=1.000 "
        "-p motion_drift_max_extra_vy:=0.040 -p motion_drift_ramp_rate:=0.030 "
