...
from .route_quality import AdaptiveRouteSampler, RouteSample, build_clean_route
...
class RouteRecorder(Node):
    def __init__(self):
        ...
        self.declare_parameter('max_record_step', 0.80)
    ...
    def odom_callback(self, msg):
        ...
        if self.last_observed_x is not None:
            ...
            if self.max_odom_step > 0.0 and observation_step > self.max_odom_step:
                ...
                # Keep the last trusted observation. An isolated bad point can
                # therefore recover on the next callback, while a persistent
                # localization offset pauses the clean route instead of joining
                # two unrelated map positions.
                return
    ...
    def rewrite_clean_route(self, samples):
        temporary_file = f'{self.route_file}.cleaning'
        with open(temporary_file, 'w', newline='') as file:
            writer = csv.writer(file)
    ...
    def finalize_route_file(self):
        ...
        try:
            ...
            self.get_logger().info(
                'final clean route saved: '
                f'raw={report["raw_samples"]}, '
                f'route={report["route_points"]}, '
                f'spikes_removed={report["isolated_spikes_removed"]}, '
            ...
            self.get_logger().error(
                f'final route cleanup failed; retained live route: {exc}'
            )
