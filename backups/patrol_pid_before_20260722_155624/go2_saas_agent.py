...
def start_patrol_command(route_path, params, route_info=None):
    ...
    sdk_receiver_cmd = "GO2_SDK_MAX_VY=0.200 %s/build/go2_cmd_vel_bridge/go2_sdk2_udp_receiver %s 5005" % (shlex.quote(WS), sdk_if_arg)
    cmd_vel_sender_cmd = (
        "ros2 run go2_cmd_vel_bridge cmd_vel_udp_sender --ros-args "
        "-p target_ip:=127.0.0.1 -p target_port:=5005 "
        "-p max_vx:=%s -p max_vy:=0.200 -p max_vyaw:=%s "
        "-p unitree_vy_sign:=1.0"
    ) % (speed_arg, max_yaw_rate_arg)
    safe_cmd = (
        "ros2 run go2_fastlio_patrol unitree_safe_cmd_node --ros-args "
        "-p max_vx:=%s -p max_vy:=0.200 -p max_yaw_rate:=%s -p publish_rate:=20.0 "
    ...
    follower_cmd = (
        ...
        "-p lateral_velocity_gain:=0.750 -p max_vy:=0.200 "
        "-p lateral_velocity_deadband:=0.030 -p lateral_velocity_heading_limit_deg:=25.0 "
        "-p lateral_velocity_pid_enabled:=true "
        "-p lateral_velocity_ki:=0.080 -p lateral_velocity_kd:=0.040 "
        "-p lateral_velocity_integral_vy_limit:=0.060 "
