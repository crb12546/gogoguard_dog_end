...
stop_exact_group() {
  ...
  local signal_name="$2"
  [[ "${pgid}" =~ ^[0-9]+$ ]] || return 0
  "${group_guard}" status "${pgid}" "${run_id}" >/dev/null 2>&1 || return 0
  "${group_guard}" signal \
    "${pgid}" "${run_id}" "SIG${signal_name}" >/dev/null 2>&1 || true
}

supervisor_matches() {
  local pid="$1"
  local recorded_start_ticks="$2"
  local actual_start_ticks
  [[ "${pid}" =~ ^[0-9]+$ && -r "/proc/${pid}/cmdline" ]] || return 1
  tr '\0' ' ' <"/proc/${pid}/cmdline" |
    grep -Fq "start_xbf_patrol.sh" || return 1
  [[ "${recorded_start_ticks}" =~ ^[0-9]+$ ]] || return 0
  actual_start_ticks="$(xbf_process_start_ticks "${pid}" 2>/dev/null)" ||
    return 1
  [[ "${actual_start_ticks}" == "${recorded_start_ticks}" ]]
}
...
supervisor_start_ticks="$(pid_value supervisor_start_ticks)"
...
[[ -x "${group_guard}" ]] || {
  echo "错误：缺少精确进程组校验器：${group_guard}" >&2
  exit 2
}
[[ "${run_id}" =~ ^xbf-[0-9a-f]{24}$ ]] || {
  echo "错误：运行记录缺少有效 run_id；拒绝按裸 PGID 停进程。" >&2
  exit 2
}

if [[ "${supervisor_pid}" =~ ^[0-9]+$ ]] &&
  kill -0 "${supervisor_pid}" 2>/dev/null &&
  supervisor_matches "${supervisor_pid}" "${supervisor_start_ticks}"; then
...
for pgid in \
  ...
  if [[ "${pgid}" =~ ^[0-9]+$ ]] &&
    ...
    "${group_guard}" signal \
      "${pgid}" "${run_id}" SIGTERM >/dev/null 2>&1 || true
  fi
done
sleep 1
...
for pgid in \
  ...
  if [[ "${pgid}" =~ ^[0-9]+$ ]] &&
    "${group_guard}" status "${pgid}" "${run_id}" >/dev/null 2>&1; then
    "${group_guard}" signal \
      "${pgid}" "${run_id}" SIGKILL >/dev/null 2>&1 || true
  fi
done
...
rm -f -- \
  "${pid_file}" \
  "${XBF_RUNTIME_DIR}/localizer.runtime.yaml" \
  "${XBF_RUNTIME_DIR}/coordinator.runtime.yaml"
