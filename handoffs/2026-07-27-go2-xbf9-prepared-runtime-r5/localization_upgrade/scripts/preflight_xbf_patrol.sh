...
默认地图：
  maps/xbf9-horizontal-clean-r1
默认路线：
  routes/xbf9_horizontal_clean.aligned.csv
默认 checkpoint：
  routes/xbf9_horizontal_clean.checkpoints.json
EOF
}
...
[[ "${RMW_IMPLEMENTATION}" == "rmw_cyclonedds_cpp" ]] ||
  xbf_fail "运行时 RMW 未固定到 Cyclone DDS：${RMW_IMPLEMENTATION:-<空>}"
[[ "${CYCLONEDDS_URI}" == "${XBF_CYCLONEDDS_URI}" ]] ||
  xbf_fail "Cyclone DDS 配置没有按部署值导出：${CYCLONEDDS_URI:-<空>}"
ros2 pkg prefix rmw_cyclonedds_cpp >/dev/null ||
  xbf_fail "当前完整 ROS 2 环境找不到 rmw_cyclonedds_cpp"
...
if not isinstance(checkpoints, list):
    raise SystemExit("checkpoint sidecar 的 checkpoints 不是数组")
...
if count:
    print(f"  严格解析通过：{len(route)} 个路线点，{count} 个中途 checkpoint")
else:
    print(
        f"  严格解析通过：{len(route)} 个路线点；当前任务只做起点校准，"
        "不设置中途 checkpoint"
    )
PY
